...
#ifndef Correction_CorrectionAssistASTVisitor_INCLUDED
#define Correction_CorrectionAssistASTVisitor_INCLUDED
...
namespace Jikes { // Open namespace Jikes block
	...
	namespace Correction {
		...
		class CorrectionAssistASTVisitor :public ASTVisitor
		{
		public:
			vector<Ast*> trackAstNodes;
			...
			enum  CompletNodeType
			{
				...
				CompletionOnExplicitThisConstructorCall,
				CompletionOnExplicitSuperConstructorCall,

				CompletionOnExplicittThisExpr,
				CompletionOnExplicittSuperExpr,

				CompletionOnTypeIdentifier,
			...
			CorrectionAssistASTVisitor(unsigned _pos);
		
			Ast* enterpoint_node = nullptr;
			bool checkTypeDeclaration(TypeDeclaration* node, AstNodeScope* scope);
		...
		inline void CorrectionAssistASTVisitor::TokenInPosThenNoFound(Token* tk)const
		{
			if (tk)
			{
				if (tk->IsInRange(pos))
