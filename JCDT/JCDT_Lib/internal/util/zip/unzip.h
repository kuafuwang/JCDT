...
public:
	Unzip();
	~Unzip();
     unsigned long global_bb;                         /* bit buffer */
     unsigned global_bk;                    /* bits in bit buffer */

     unsigned global_wp;  /* current position in slide */
     unsigned global_hufts; /* huff memory usage */
     unsigned char* slide_buffer;
     struct huft *global_fixed_tl;    /* inflate static */
     struct huft *global_fixed_td;    /* inflate static */
     int global_fixed_bl,
               global_fixed_bd;
#ifdef UNIX_FILE_SYSTEM
    static FILE *global_file; /* file pointer for zip file */
     ...
     char *global_file;
#endif
     char *global_bufferp; /* current position in output buffer */
    ...
    //
	 ...
	 int(Unzip::*uncompress_file[10]) (char *, char *, long);
     int huft_build(unsigned *b, unsigned n, unsigned s,
...
#ifdef UNIX_FILE_SYSTEM
     int unzip8(FILE * zipfile, char *buffer);

     int UncompressFile0(FILE *, char *, long);
     int UncompressFile1(FILE *, char *, long);
     int UncompressFile2(FILE *, char *, long);
     int UncompressFile3(FILE *, char *, long);
     int UncompressFile4(FILE *, char *, long);
     int UncompressFile5(FILE *, char *, long);
     int UncompressFile6(FILE *, char *, long);
     int UncompressFile7(FILE *, char *, long);
     int UncompressFile8(FILE *, char *, long);
     int UncompressFile9(FILE *, char *, long);
#elif defined(WIN32_FILE_SYSTEM)
     int unzip8(char *zipfile, char *buffer);

     int UncompressFile0(char *, char *, long);
     int UncompressFile1(char *, char *, long);
     int UncompressFile2(char *, char *, long);
     int UncompressFile3(char *, char *, long);
     int UncompressFile4(char *, char *, long);
     int UncompressFile5(char *, char *, long);
     int UncompressFile6(char *, char *, long);
     int UncompressFile7(char *, char *, long);
     int UncompressFile8(char *, char *, long);
     int UncompressFile9(char *, char *, long);
#endif
}; // end class unzip
