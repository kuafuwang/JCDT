...
{
    for (int head = 0, tail = type_list.size() - 1;
         head < tail; head++, tail--)
    {
...
void TypeCycleChecker::PartialOrder(vector<Semantic*>& semantic, int start)
{
    type_list.clear();
    ...
    // set to OMEGA
    //
    for (unsigned i = start; i < semantic.size(); i++)
    {
        Semantic* sem = semantic[i];
        for (unsigned k = 0;
             k < sem -> compilation_unit -> NumTypeDeclarations(); k++)
        {
            TypeDeclaration* declared =
                sem -> compilation_unit -> GetTypeDeclaration(k);
            if (EmptyTypeDeclaration::EmptyDeclarationCast(declared))
                continue;
    ...
    if (type -> index == indx)
    {
        ...
        do
        {
            ...
            *(scc_subtype -> subtypes_closure) = *(type -> subtypes_closure);
            type_list.push_back(scc_subtype);
            stack.Pop();
...
void ConstructorCycleChecker::CheckConstructorCycles(ConstructorDeclaration* constructor_declaration)
{
    stack.Push(constructor_declaration);
    int indx = stack.Size();
    constructor_declaration -> index = indx;

    ConstructorDeclaration* called_constructor_declaration = NULL;
    ...
    if (constructor_block -> explicit_constructor_opt)
    {
        ...
        if (called_constructor)
        {
            called_constructor_declaration =
                (ConstructorDeclaration*) called_constructor -> declaration;
        ...
        else
        {
            do
            {
                ...
                AstMethodDeclarator* constructor_declarator =
                    called_constructor_declaration ->method_declarator;

                Semantic* sem = called_constructor_declaration ->
                    method_symbol -> containing_type ->
                    semantic_environment -> sem;
                sem -> ReportSemErrorWithAst(SemanticError::CIRCULAR_THIS_CALL,
...
void TypeDependenceChecker::PartialOrder()
{
    ...
    {
        for (unsigned j = 0; j < file_symbol -> types.size(); j++)
        {
    ...
    for (unsigned k = 0; k < type_trash_bin.size(); k++)
    {
                                       ...
                                       Tuple<FileSymbol*>& file_list)
{
    ...
    for (unsigned i = 0; i < file_list.Length(); i++)
    {
       ...
       const char* name = file_symbol -> FileName();
...
void TypeDependenceChecker::OutputMake(FileSymbol* file_symbol)
{
    ...
    else
    {
        ...
        auto dir_name = dir_symbol -> DirectoryName();
        int dir_length = dir_symbol->DirectoryNameLength();

        buf = new char[length + FileSymbol::class_suffix_length + dir_length +
                      2];
        memcpy(buf, dir_name, dir_length);
                             ...
                             bool static_access)
{
    ...
    if (base_type -> Bad() || parent_type -> Bad() ||
        parent_type == lookup_env.null_type || parent_type -> Primitive())
    {
        return;
    }
    ...
    //
    assert(parent_type -> ContainingPackage() != lookup_env.UnnamedPackage() ||
           base_type -> ContainingPackage() == lookup_env.UnnamedPackage());
}
