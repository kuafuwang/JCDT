...
namespace Jikes { // Open namespace Jikes block
	...
	//
	static void FormatClasspath(ErrorString& s, Tuple<PathSymbol*>&classpath)
	{
		s << endl;
...
//
	...
	SemanticError::SemanticError(Option* option_, 
		Tuple<PathSymbol*>& classpath_,
		...
		errors()
	{
		ErrorInfo::emacs_style_report = !option->errors;
	}
	...
	void SemanticError::Report(SemanticErrorKind msg_code,
		Token* left_token, Token* right_token,
		...
		const wchar_t* insert9)
	{
		...
		case WEAK_WARNING:
			warning[msg_code] =
				option->tolerance & JikesOption::WARNINGS_ARE_ERRORS
				? MANDATORY_ERROR : WEAK_WARNING;
			break;
		case NAMED_STRONG_ON:
		case STRONG_WARNING:
			warning[msg_code] =
				option->tolerance & JikesOption::CAUTIONS_ARE_ERRORS
		...
		errors.push_back(new ErrorInfo());
		int i = errors.size() - 1;
		...
		errors[i]->severity = (Problem::Severity) warning[msg_code];
		...
		if (left_token && right_token) {
			errors[i]->left_token = (left_token->index > right_token->index
				? right_token : left_token);
		}



		errors[i]->right_token = right_token;
	}

	static Tuple<MessageGroup*> message_groups;


	void SemanticError::DestoryStaticMessages()
	{
		size_t size = message_groups.Length();
		...
		while (top >= 0)
		{
			...
			while (upper > lower)
			{
				...
				for (j = lower + 1; j <= upper; j++)
					if ((errors[j]->LeftIndex() < pivot->LeftIndex()) ||
						//
						...
						//
						(errors[j]->LeftIndex() == pivot->LeftIndex() &&
							errors[j]->RightIndex() > pivot->RightIndex()) ||
						//
						...
						(errors[j]->LeftIndex() == pivot->LeftIndex()  &&
							errors[j]->RightIndex() == pivot->RightIndex() &&
							pivot->LeftIndex() == pivot->RightIndex()     &&
	...
	int SemanticError::PrintMessages()
	{
		...
		if (option->errors) // regular error messages
		{
			if (num_errors == 0)
			{
				if (option->tolerance == JikesOption::NO_WARNINGS)
				...
				Coutput << endl << "Issued " << num_warnings
					<< (file_symbol->semantic->IsSystermSemantic() ? " system" : " semantic")
			...
			else // we had some errors, and possibly warnings as well
			{
				...
				if (num_warnings > 0 &&
					option->tolerance != JikesOption::NO_WARNINGS)
				{
			...
			if (!file_symbol->semantic->IsSystermSemantic())
			{
				Coutput << " compiling \"" << file_symbol->FileName() << '\"';
			}
			Coutput << ':';
		}
		...
		if (!file_symbol->semantic->IsSystermSemantic())
		{
			...
			{
				auto file_name = lex_stream->FileName();
				int length = lex_stream->FileNameLength();
				wchar_t *name = new wchar_t[length + 1];
				for (int i = 0; i < length; i++)
					name[i] = file_name[i];
				name[length] = U_NULL;
				file_symbol->semantic ->ReportSemErrorSimple(SemanticError::CANNOT_REOPEN_FILE,BAD_TOKEN, name);
				delete[] name;
			}
		}
			...
			lex_stream->InputBuffer())
		{
			...
			for (unsigned k = 0; k < errors.size(); k++){
				if (warning[errors[k]->msg_code] != 1 ||
					option->tolerance != JikesOption::NO_WARNINGS)
				{
					reportError(k);
				}
	...
	int SemanticError::InitAllMessages()
	{
		int return_code = (num_errors > 0 ? 1 : 0);
	
		// Reopen the file to report the errors, unless we didn't parse it in the first place.
		auto lex_stream = file_symbol->lex_stream;
		if (!file_symbol->semantic->IsSystermSemantic()){
			lex_stream->RereadInput();
			if (!lex_stream->InputBuffer()){
			...
			lex_stream->InputBuffer())
		{
			//SortMessages();
			for (unsigned k = 0; k < errors.size(); k++){
				if (errors[k]->initialized == false) {
					if (warning[errors[k]->msg_code] != 1 ||
						option->tolerance != JikesOption::NO_WARNINGS) {
						FormatError(*errors[k]);
	...
	void SemanticError::Clear()
	{
		
		size_t _size = errors.size();
	...
	void SemanticError::TransferError(vector<JikesError*>& errer_owner){
		InitAllMessages();
		vector<ErrorInfo*> temp;
		temp.swap(errors);
		Clear();
		...
		while (*p)
		{
			...
			else
			{
				...
				case 'F': // A .class or .java file name
					...
					break;
				case 'L':
					s << file_symbol->lex_stream->FileName();
					break;
				case 'P':
	...
	{
		FormatError(*errors[k]);
		errors[k]->Initialize(file_symbol->lex_stream);
	...
	//
	// There are two steps:
	//   1. Add an enum value to the SemanticErrorKind enum in 'JCDT_Lib/internal/problem/error.h'.
	...
	void SemanticError::InitializeMessages()
	{
		...
		messages[PACKAGE_NOT_FOUND] =
			"You need to modify your class paths, "
