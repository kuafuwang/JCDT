...
void Operators::OpLine(Ostream& os, const ConstantPool& constant_pool, char hdr, int pc,
                       ...
                       OpInfo info_kind, unsigned info_index)
{
    ...
    case INFO_CONST:
        ...
        {
            os << '(';
            constant_pool[info_index] -> Describe(os,constant_pool);
            os << ')';
        }
        else os << "(illegal)";
        break;
...
class OperatorsEx : public  Operators 
	{
public:
	static void OpDmp(Ostream& os, const ConstantPool& constant_pool, const Tuple<u1>& code, std::vector<std::pair<int, int>>& codePostion)
	{
		...
		while (pc < code.Length())
		{
			...
			codePostion.emplace_back(std::make_pair(pc_start, os.lineNo));
			...
			{
				os.width(5);
				os << pc_start << "  <incomplete: " << name << '>' << endl;
			...
			case OP_TABLESWITCH:
			{
				...
				OpLine(os, constant_pool, ' ', pc_start, name,
				...
				while (len)
				{
					...
					OpLine(os, constant_pool, '*', pc_this, name,
			...
			case OP_LOOKUPSWITCH:
			{
				...
				while (npairs)
				{
					...
					OpLine(os, constant_pool, '*', pcb, name,
						argdesc, desc, INFO_NONE, 0);
					npairs--;
				}
				info_kind = INFO_DONE;
			...
			case OP_WIDE:
			{
				...
				{
					os.width(5);
					os << pc_start << "  <incomplete: wide " << name
						<< '>' << endl;
				...
				default:
					os.width(5);
					os << pc_start << "  <illegal wide instruction: "
						<< name << '>' << endl;
...
void OperatorsEx_OpDmp(Ostream& os, const ConstantPool& constant_pool, const Tuple<u1>& code, std::vector<std::pair<int, int>>& codePostion)
{
	OperatorsEx::OpDmp(os, constant_pool, code, codePostion);
}
//
