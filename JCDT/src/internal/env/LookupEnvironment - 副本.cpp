...
void	LookupEnvironment::Complier(char** arguments)
{
    //
         ...
         file_symbol = (FileSymbol*) input_java_file_set.NextElement())
    {
        ...
        if (file_symbol -> lex_stream) // did we have a successful scan!
        {
			...
			reporter->parser_error_ = new ParseError(option, file_symbol->lex_stream);
			reporter->semantic_error_ = new SemanticError(option, classpath, file_symbol);
			reporter->option = option;
			file_symbol->problemReporter_ = reporter;
			
			AstPackageDeclaration* package_declaration =
				
			parser-> PackageHeaderParse(file_symbol,
                                             ast_pool);
	...
	ProblemReporter* reporter = new ProblemReporter();
	reporter->semantic_error_ = new SemanticError(option, classpath, main_file_clone);
	reporter->option = option;
	main_file_clone->problemReporter_ = reporter;
    ...
    {
        system_semantic -> ReportSemErrorWithAst(SemanticError::NO_CURRENT_DIRECTORY,
			nullptr,nullptr);
    }
    ...
    {
        system_semantic ->ReportSemErrorSimple(SemanticError::CANNOT_OPEN_PATH_DIRECTORY, BAD_TOKEN, bad_dirnames[i]);
    }
    ...
    for (i = 0; i < bad_input_filenames.Length(); i++)
    {
        system_semantic -> ReportSemErrorWithAst(SemanticError::BAD_INPUT_FILE,
			nullptr,nullptr, bad_input_filenames[i]);
    }
    ...
    //else
    {
        ...
        if (option->incremental)
        {
                 ...
                 recompile; recompile = IncrementalRecompilation())
            {
                ...
                {
                    system_semantic ->
                        ReportSemErrorWithAst(SemanticError::BAD_INPUT_FILE, nullptr,
                                       nullptr, bad_input_filenames[i]);
                }
                     ...
                     file_symbol = (FileSymbol*) recompilation_file_set.NextElement())
                {
                    ...
                    if (lex_stream)
                    {
                        AstPackageDeclaration* package_declaration = parser ->
                            PackageHeaderParse(file_symbol, ast_pool);
...
//
DirectorySymbol* LookupEnvironment::FindSubdirectory(PathSymbol* path_symbol,
...
DirectorySymbol* Compiler::ProcessSubdirectories(wchar_t* source_name,
...
DirectorySymbol* LookupEnvironment::ProcessSubdirectories(wchar_t* source_name,
                                                int source_name_length,
                                                bool source_dir)
{
...
void LookupEnvironment::ProcessNewInputFiles(SymbolSet& file_set, char** arguments)
{
    unsigned i;
    for (i = 0; i < bad_input_filenames.Length(); i++)
        delete [] bad_input_filenames[i];
...
FileSymbol* LookupEnvironment::FindOrInsertJavaInputFile(DirectorySymbol* directory_symbol,
...
FileSymbol* LookupEnvironment::FindOrInsertJavaInputFile(wchar_t* name, int name_length)
{
    FileSymbol* file_symbol = NULL;
                                            ...
                                            AstName* name)
{
    ...
    if (name -> base_opt)
    {
        ...
        NameSymbol* name_symbol =
            lex_stream -> GetNameSymbol(name -> identifier_token->index);
...
void LookupEnvironment::ProcessHeaders(FileSymbol* file_symbol)
{
    ...
    if (file_symbol -> lex_stream) // do we have a successful scan!
    {
        if (! file_symbol -> compilation_unit)
            file_symbol -> compilation_unit =
                parser -> HeaderParse(file_symbol);
        //
...
void LookupEnvironment::ProcessMembers()
{
...
void LookupEnvironment::CollectTypes(TypeSymbol* type, Tuple<TypeSymbol*>& types)
{
    types.Next() = type;
...
void LookupEnvironment::ProcessBodies(TypeSymbol* type)
{
        ...
        ! sem -> compilation_unit -> BadCompilationUnitCast())
    {
        ...
        if (! type -> file_symbol -> IsZip())
        {
            ...
            if (entry && strcmp(classfile_name, entry -> name) != 0)
            {
                ...
                sem -> ReportSemError(SemanticError::FILE_FILE_CONFLICT,
                                      type -> declaration -> identifier_token->index,
									  type->declaration->identifier_token->index,
        ...
        if (! parser -> InitializerParse(sem -> source_file_symbol,
                                         type -> declaration))
        {
        ...
        else
        {
            ...
            if (! parser -> BodyParse(sem -> source_file_symbol, type -> declaration))
            {
                // Mark that syntax errors were detected.
                sem -> compilation_unit -> MarkBad();
            }
                                        ...
                                        AstPackageDeclaration* package_declaration)
{
    ...
    for (unsigned i = 0; i < file_symbol -> lex_stream -> NumTypes(); i++)
    {
        ...
        {
            NameSymbol* name_symbol =
                file_symbol -> lex_stream -> GetNameSymbol(identifier_token);
