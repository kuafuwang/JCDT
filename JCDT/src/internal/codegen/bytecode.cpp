...
void ByteCode::GenerateCode(Ostream& os)
{
    ...
    {
        FieldDeclaration* field_decl = class_body -> ClassVariable(i);
        for (unsigned vi = 0;
    ...
    {
        FieldDeclaration* field_decl  = class_body -> InstanceVariable(i);
        for (unsigned vi = 0;
             vi < field_decl -> NumVariableDeclarators(); vi++)
        {
            AstVariableDeclarator* vd = field_decl -> VariableDeclarator(vi);
            ...
            if (vd -> variable_initializer_opt && vsym -> initial_value)
            {
                Expression* init;
				assert(init =
					Expression::ExpressionCast(vd->variable_initializer_opt));
    ...
    if (assert_variable)
    {
        assert(! lookupEnv.option->noassert);
        DeclareField(assert_variable);
        if (lookupEnv.option->target < JikesOption::SDK1_4)
        {
            semantic.ReportSemErrorWithAst(SemanticError::ASSERT_UNSUPPORTED_IN_TARGET,
    ...
    {
        MethodDeclaration* method = class_body -> Method(i);
    ...
    for (i = 0; i < unit_type -> NumPrivateAccessMethods(); i++)
    {
        ...
        MethodDeclaration* method =
			MethodDeclaration::MethodDeclarationCast(method_sym->declaration);
        assert(method);
    ...
    for (i = 0; i < unit_type -> NumPrivateAccessConstructors(); i++)
    {
        ...
        ConstructorDeclaration* constructor =
            ConstructorDeclaration::ConstructorDeclarationCast(constructor_sym->declaration);
    ...
    {
        semantic.ReportSemErrorWithAst(SemanticError::CONSTANT_POOL_OVERFLOW,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    if (interfaces.Length() > 65535)
    {
        ...
        semantic.ReportSemErrorWithAst(SemanticError::INTERFACES_OVERFLOW,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    if (fields.Length() > 65535)
    {
        ...
        semantic.ReportSemErrorWithAst(SemanticError::FIELDS_OVERFLOW,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    if (methods.Length() > 65535)
    {
        ...
        semantic.ReportSemErrorWithAst(SemanticError::METHODS_OVERFLOW,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    if (string_overflow)
    {
        semantic.ReportSemErrorWithAst(SemanticError::STRING_OVERFLOW,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    ...
    {
        semantic.ReportSemErrorWithAst(SemanticError::LIBRARY_METHOD_NOT_FOUND,
                                unit_type -> declaration,
                                unit_type -> ContainingPackageName(),
                                unit_type -> ExternalName());
    }
    ...
    //if (semantic.NumErrors() == 0)
        Write(os,unit_type);
#ifdef JIKES_DEBUG
    if (lookupEnv.option->debug_dump_class)
        Print(os);
#endif // JIKES_DEBUG
}
...
//
void ByteCode::CompileConstructor(ConstructorDeclaration* constructor,
                                  Tuple<AstVariableDeclarator*>& constants,
                                  bool has_instance_initializer)
{
    MethodSymbol* method_symbol = constructor -> method_symbol;
    ...
    if (lookupEnv.option->target < JikesOption::SDK1_4)
    {
        ...
        if (constructor_block -> explicit_constructor_opt)
            ...
            assert(unit_type == lookupEnv.Object() &&
    ...
    //
    if (constructor_block -> explicit_constructor_opt &&
        ! AstThisCall::ThisCallCast(constructor_block->explicit_constructor_opt))
    {
        if (unit_type -> EnclosingType())
        {
            //
             ...
             i < unit_type -> NumConstructorParameters(); i++)
        {
            ...
            PutOp(OP_PUTFIELD);
            if (lookupEnv.IsDoubleWordType(shadow -> Type()))
                ChangeStack(-1);
...
void ByteCode::DeclareField(VariableSymbol* symbol)
{
    ...
    {
        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW,
                                symbol -> declarator);
    }
...
void ByteCode::BeginMethod(int method_index, MethodSymbol* msym)
{
...
#endif // DUMP
#ifdef JIKES_DEBUG
    if (lookupEnv.option->debug_trace_stack_change)
    ...
    if (msym -> NumThrows())
    {
        ExceptionsAttribute* exceptions_attribute =
            new ExceptionsAttribute(RegisterUtf8(lookupEnv.nameEnv->Exceptions_literal));
    ...
    if (! (msym -> ACC_ABSTRACT() || msym -> ACC_NATIVE()))
    {
        ...
        code_attribute =
            new CodeAttribute(RegisterUtf8(lookupEnv.nameEnv->Code_literal),
                              msym -> block_symbol -> max_variable_index);
        line_number = 0;
        line_number_table_attribute = new LineNumberTableAttribute
            (RegisterUtf8(lookupEnv.nameEnv->LineNumberTable_literal));

        local_variable_table_attribute = (lookupEnv.option->g & JikesOption::VARS)
            ? (new LocalVariableTableAttribute
               (RegisterUtf8(lookupEnv.nameEnv->LocalVariableTable_literal)))
            : (LocalVariableTableAttribute*) NULL;
    }

    if (msym -> Type() -> num_dimensions > 255)
    {
        assert(MethodDeclaration::MethodDeclarationCast(msym->declaration));
        Ast* type = ((MethodDeclaration*) msym -> declaration) -> type;

        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW, type);
    }
    ...
    if (parameter)
    {
        ...
        if (lookupEnv.IsDoubleWordType(parameter -> Type()))
        ...
        if (last_parameter_index >= 255)
        {
            ...
            MethodDeclaration* method_declaration =
               MethodDeclaration::MethodDeclarationCast(msym->declaration);
            ConstructorDeclaration* constructor_declaration =
                ConstructorDeclaration::ConstructorDeclarationCast(msym->declaration);
...
void ByteCode::EndMethod(int method_index, MethodSymbol* msym)
{
    ...
    if (! (msym -> ACC_ABSTRACT() || msym -> ACC_NATIVE()))
    {
        ...
        {
            semantic.ReportSemErrorWithAst(SemanticError::LOCAL_VARIABLES_OVERFLOW,
        ...
        if (max_stack > 65535)
        {
            semantic.ReportSemErrorWithAst(SemanticError::STACK_OVERFLOW,
                                    msym -> declaration, msym -> Header(),
                                    unit_type -> ContainingPackageName(),
                                    unit_type -> ExternalName());
        }
        ...
        {
            semantic.ReportSemErrorWithAst(SemanticError::CODE_OVERFLOW,
                                    msym -> declaration, msym -> Header(),
                                    unit_type -> ContainingPackageName(),
                                    unit_type -> ExternalName());
        }
        ...
        //
        if ((lookupEnv.option->g & JikesOption::LINES) &&
        ...
        //
        if ((lookupEnv.option->g & JikesOption::VARS)
            && (! msym -> accessed_member)
            && (msym -> Identity() != lookupEnv.nameEnv->class_name_symbol))
        {
            if (! msym -> ACC_STATIC()) // add 'this' to local variable table
            {
                local_variable_table_attribute ->
                    AddLocalVariable(0, code_attribute -> CodeLength(),
                                     RegisterUtf8(lookupEnv.nameEnv->this_name_symbol -> Utf8_literal),
...
void ByteCode::InitializeVariable(AstVariableDeclarator* vd)
{
    ...
    else
    {
        AstArrayInitializer* array_initializer =
            AstArrayInitializer::ArrayInitializerCast(vd->variable_initializer_opt);
    ...
    if (expression && lookupEnv.IsDoubleWordType(expression -> Type()))
        ChangeStack(-1);
                               ...
                               bool need_value)
{
         ...
         i < array_initializer -> NumVariableInitializers(); i++)
    {
        ...
        Expression* expr = Expression::ExpressionCast(entry);
        if (expr && (IsZero(expr) || expr -> Type() == lookupEnv.null_type))
        {
            bool optimize;
            if (expr -> Type() == lookupEnv.float_type)
            {
                FloatLiteralValue* value = DYNAMIC_CAST<FloatLiteralValue*>
                    (expr -> value);
                optimize = value -> value.IsPositiveZero();
            }
            else if (expr -> Type() == lookupEnv.double_type)
            {
        ...
        {
            assert(AstArrayInitializer::ArrayInitializerCast(entry));
            InitializeArray(subtype, AstArrayInitializer::ArrayInitializerCast(entry),
                            need_value);
        }
        if (need_value)
...
void ByteCode::DeclareLocalVariable(AstVariableDeclarator* declarator)
{
    ...
    if (type -> num_dimensions > 255)
        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW, declarator);
    ...
    {
		AstArrayCreationExpression* ace = AstArrayCreationExpression::ArrayCreationExpressionCast(declarator ->
			variable_initializer_opt);
        AstArrayInitializer* ai = AstArrayInitializer::ArrayInitializerCast(declarator
			->variable_initializer_opt);
        if (ace)
        ...
        {
            Expression* expr =
                (Expression*) declarator -> variable_initializer_opt;
	        assert(Expression::ExpressionCast(declarator->variable_initializer_opt));
            EmitExpression(expr);
            //
            ...
            //
            if (lookupEnv.option->target < JikesOption::SDK1_5 &&
                IsMultiDimensionalArray(type) &&
                (StripNops(expr) -> Type() == lookupEnv.null_type))
            {
                PutOp(OP_CHECKCAST);
...
//
bool ByteCode::EmitStatement(ReachComleteAbleStatement* statement)
{
    if (! AstBlock::BlockCast(statement))
    {
        line_number_table_attribute ->
            AddLineNumber(code_attribute -> CodeLength(),
                          semantic.lex_stream -> LineFromPos(statement -> LeftToken()->start_location));
    }
    ...
    case Ast::LOCAL_VARIABLE_DECLARATION: // JLS 14.3
        {
            LocalDeclaration* lvs =
                LocalDeclaration::LocalVariableStatementCast(statement);
    ...
    case Ast::EXPRESSION_STATEMENT: // JLS 14.7
        EmitStatementExpression(AstExpressionStatement::ExpressionStatementCast(statement) ->
                                expression);
        return false;
    ...
    case Ast::SWITCH: // JLS 14.9
        return EmitSwitchStatement(SwitchStatement::SwitchStatementCast(statement));
    ...
    case Ast::WHILE: // JLS 14.10
        {
            WhileStatement* wp = WhileStatement::WhileStatementCast(statement);
            bool abrupt = false;
            //
            ...
            if (! wp -> statement -> can_complete_normally)
            {
                ...
                else
                {
                    line_number_table_attribute ->
                        AddLineNumber(code_attribute -> CodeLength(),
                                      semantic.lex_stream -> LineFromPos(wp -> expression -> LeftToken()->start_location));
    ...
    case Ast::DO: // JLS 14.11
        {
            DoStatement* sp = DoStatement::DoStatementCast(statement);
            Label begin_label;
            DefineLabel(begin_label);
            ...
            if (! abrupt)
            {
                ...
                line_number_table_attribute ->
                    AddLineNumber(code_attribute -> CodeLength(),
                                  semantic.lex_stream -> LineFromPos(sp -> expression -> LeftToken()->start_location));
    ...
    case Ast::FOR: // JLS 14.12
        {
            ForStatement* for_statement = ForStatement::ForStatementCast(statement);
            bool abrupt = false;
            ...
            if (! for_statement -> statement -> can_complete_normally)
            {
                ...
                if (for_statement -> end_expression_opt)
                {
                    ...
                    else
                    {
                        ...
                        line_number_table_attribute ->
                            AddLineNumber(code_attribute -> CodeLength(),
                                          semantic.lex_stream -> LineFromPos(for_statement -> end_expression_opt -> LeftToken()->start_location));
            ...
            Expression* end_expr = for_statement -> end_expression_opt;
            if (end_expr)
            {
                assert(stack_depth == 0);
                ...
                line_number_table_attribute ->
                    AddLineNumber(code_attribute -> CodeLength(),
                                  semantic.lex_stream -> LineFromPos(end_expr ->
                                                              LeftToken()->start_location));
    ...
    case Ast::BREAK: // JLS 14.13
        {
            unsigned nesting_level =
                BreakStatement::BreakStatementCast(statement) -> nesting_level;
            AstBlock* over = method_stack -> GetBlock(nesting_level);
    ...
    case Ast::CONTINUE: // JLS 14.14
        {
            unsigned nesting_level =
                ContinueStatement::ContinueStatementCast(statement) -> nesting_level;
            AstBlock* over = method_stack -> GetBlock(nesting_level);
            u2 jump_size = (over -> RightToken() - over -> LeftToken() <
                            TOKEN_WIDTH_REQUIRING_GOTOW) ? 3 : 5;
            if (ProcessAbruptExit(nesting_level, jump_size))
    ...
    case Ast::RETURN: // JLS 14.15
        EmitReturnStatement(ReturnStatement::ReturnStatementCast(statement));
        return true;
    case Ast::SUPER_CALL:
    ...
    case Ast::THROW: // JLS 14.16
        EmitExpression(ThrowStatement::ThrowStatementCast(statement) -> expression);
        PutOp(OP_ATHROW);
        return true;
    case Ast::SYNCHRONIZED_STATEMENT: // JLS 14.17
        return EmitSynchronizedStatement((SynchronizedStatement*) statement);
    case Ast::TRY_KIND: // JLS 14.18
        EmitTryStatement((TryStatement*) statement);
    ...
    case Ast::FINALLY: // JLS 14.18
        ...
        return false;
    case Ast::ASSERT_KIND: // JDK 1.4 (JSR 41)
        EmitAssertStatement((AssertStatement*) statement);
        return false;
...
void ByteCode::EmitReturnStatement(ReturnStatement* statement)
{
    Expression* expression = statement -> expression_opt;
    ...
    else
    {
        ...
        assert(type != lookupEnv.void_type);
...
bool ByteCode::EmitBlockStatement(AstBlock* block)
{
    ...
    for (unsigned i = 0; i < block -> NumStatements() && ! abrupt; i++)
        abrupt = EmitStatement(block -> GetStatement(i));
...
void ByteCode::EmitStatementExpression(Expression* expression)
{
...
// }
//
bool ByteCode::EmitSwitchStatement(SwitchStatement* switch_statement)
{
    ...
    if (switch_statement -> expression -> IsConstant())
    {
        ...
        for (i = 0; i < target -> block_index; i++)
            EmitSwitchBlockStatement(switch_statement -> GetBlock(i), true);
        abrupt = false;
        for ( ; ! abrupt && i < switch_statement -> NumBlocks(); i++)
        {
            abrupt =
                EmitSwitchBlockStatement(switch_statement -> GetBlock(i), abrupt);
        }
        ...
        switch_statement -> DefaultCase())
    {
        EmitExpression(switch_statement -> expression, false);
        method_stack -> Push(switch_block);
        abrupt = EmitSwitchBlockStatement(switch_statement -> GetBlock(0), false);
        CloseSwitchLocalVariables(switch_block, op_start);
        if (IsLabelUsed(method_stack -> TopBreakLabel()))
        {
            abrupt = false;
    ...
    if (switch_statement -> NumCases() == 1)
    {
        ...
        if (! switch_statement -> DefaultCase())
        {
            ...
            method_stack -> Push(switch_block);
            EmitSwitchBlockStatement(switch_statement -> GetBlock(0), false);
            CloseSwitchLocalVariables(switch_block, op_start);
            if (IsLabelUsed(method_stack -> TopBreakLabel()))
            {
                DefineLabel(method_stack -> TopBreakLabel());
    ...
    if (ncases)
    {
        ...
        //
        if (high < 0x7ffffff0L ||
            lookupEnv.option->target >= JikesOption::SDK1_4)
        {
    ...
    line_number_table_attribute ->
        AddLineNumber(code_attribute -> CodeLength(),
                      semantic.lex_stream -> LineFromPos(switch_statement ->
											expression -> LeftToken()->start_location));
    EmitExpression(switch_statement -> expression);
                                        ...
                                        bool abrupt)
{
    for (unsigned i = 0; i < block -> NumStatements(); i++)
    {
        if (! abrupt)
            abrupt = EmitStatement(block -> GetStatement(i));
        else if (LocalDeclaration::LocalVariableStatementCast(block->GetStatement(i)))
        {
            //
            ...
            //
            LocalDeclaration* lvs =
                (LocalDeclaration*) block -> GetStatement(i);
...
//
void ByteCode::EmitTryStatement(TryStatement* statement)
{
    //
    ...
    if (start_try_block_pc != end_try_block_pc)
    {
        ...
        for (unsigned i = 0; i < statement -> NumCatchClauses(); i++)
        {
            ...
            line_number_table_attribute ->
                AddLineNumber(code_attribute -> CodeLength(),
                              semantic.lex_stream -> LineFromPos(catch_clause ->
                                                          catch_token->start_location));
            //
        ...
        if (emit_finally_clause)
        {
            ...
            if (statement -> finally_clause_opt -> block ->
                can_complete_normally)
            {
                StoreLocal(variable_index, lookupEnv.Throwable()); // Save,
                EmitBranch(OP_JSR, finally_label, statement);
                special_end_pc = code_attribute -> CodeLength();
                LoadLocal(variable_index, lookupEnv.Throwable()); // reload, and
                ...
                can_complete_normally)
            {
                ...
                StoreLocal(variable_index + 1, lookupEnv.Object());
            }
                                 ...
                                 TypeSymbol* return_type)
{
    ...
    if (return_type)
    {
             ...
             i > 0 && method_stack -> NestingLevel(i) != level; i--)
        {
            ...
            AstBlock* block = method_stack -> GetBlock(nesting_level);
            if (block -> Tag() == AstBlock::TRY_CLAUSE_WITH_FINALLY)
            {
                variable_index = method_stack -> GetBlock(enclosing_level) ->
                    block_symbol -> helper_variable_index + 2;
            }
            else if (block -> Tag() == AstBlock::ABRUPT_TRY_FINALLY)
            {
                variable_index = -1;
                PutOp(lookupEnv.IsDoubleWordType(return_type) ? OP_POP2 : OP_POP);
                break;
            }
        }
    }
         ...
         i > 0 && method_stack -> NestingLevel(i) != level; i--)
    {
        ...
        if (block -> Tag() == AstBlock::TRY_CLAUSE_WITH_FINALLY)
        {
            EmitBranch(OP_JSR, method_stack -> FinallyLabel(enclosing_level),
                       method_stack -> GetBlock(enclosing_level));
        ...
        else if (block -> Tag() == AstBlock::SYNCHRONIZED)
        {
            ...
            //
            int variable_index = method_stack -> GetBlock(enclosing_level) ->
                block_symbol -> helper_variable_index;
            LoadLocal(variable_index, lookupEnv.Object());
...
void ByteCode::EmitBranch(Opcode opc, Label& lab, ReachComleteAbleStatement* over)
{
...
//
void ByteCode::EmitBranchIfExpression(Expression* p, bool cond, Label& lab,
                                      ReachComleteAbleStatement* over)
{
    p = StripNops(p);
    assert(p -> Type() == lookupEnv.boolean_type);
    ...
    AstPreUnaryExpression* pre = AstPreUnaryExpression::PreUnaryExpressionCast(p);
    if (pre) // must be !
    {
        //
        // branch_if(!e,c,l) => branch_if(e,!c,l)
        //
        assert(pre -> Tag() == PreUnaryExpressionEnum::NOT);
        EmitBranchIfExpression(pre -> expression, ! cond, lab, over);
        return;
    }

    ConditionalExpression* conditional = ConditionalExpression::ConditionalExpressionCast(p);
    if (conditional)
    {
    ...
    InstanceofExpression* instanceof = InstanceofExpression::InstanceofExpressionCast(p);
    if (instanceof)
    {
        Expression* expr = StripNops(instanceof -> expression);
        ...
        if (left_type == lookupEnv.null_type)
        {
            //
            ...
            if (! cond)
                EmitBranch(OP_GOTO, lab, over);
                 ...
                 BinaryExpression::BinaryExpressionCast(expr)) // a String concat
        {
            //
            // We know the result: true, since the expression is non-null
            // and String is a final class.
            //
            assert( lookupEnv.IsString(left_type) );
            EmitExpression(expr, false);
            if (cond)
                EmitBranch(OP_GOTO, lab, over);
        }
        else if ((AstThisExpression::ThisExpressionCast(expr) ||
                  AstSuperExpression::SuperExpressionCast(expr) ||
                  ClassLiteralAccess::ClassLiteralCast(expr) ||
                  AstClassCreationExpression::ClassCreationExpressionCast(expr) ||
                  AstArrayCreationExpression::ArrayCreationExpressionCast(expr)) &&
    ...
    //
    BinaryExpression* bp = BinaryExpression::BinaryExpressionCast(p);
    if (! bp)
    {
        EmitExpression(p);
        EmitBranch((cond ? OP_IFNE : OP_IFEQ), lab, over);
    ...
    //
    Expression* left = StripNops(bp -> left_expression);
    Expression* right = StripNops(bp -> right_expression);

    TypeSymbol* left_type = left -> Type();
    TypeSymbol* right_type = right -> Type();
    switch (bp -> Tag())
    {
    case BinaryExpressionEnum::AND_AND:
        //
        ...
        return;
    case BinaryExpressionEnum::OR_OR:
        //
        ...
        return;
    case BinaryExpressionEnum::XOR: // ^ on booleans is equavalent to !=
        assert(left_type == lookupEnv.boolean_type);
        // Fallthrough!
    case BinaryExpressionEnum::EQUAL_EQUAL:
    case BinaryExpressionEnum::NOT_EQUAL:
        //
        // One of the operands is null. We must evaluate both operands, to get
        // any side effects in (expr ? null : null).
        //
        if (left_type == lookupEnv.null_type || right_type == lookupEnv.null_type)
        {
            EmitExpression(left, left_type != lookupEnv.null_type);
            EmitExpression(right, right_type != lookupEnv.null_type);
            if (left_type == right_type)
            {
                if (cond == (bp -> Tag() == BinaryExpressionEnum::EQUAL_EQUAL))
                {
                    EmitBranch(OP_GOTO, lab, over);
                }
            }
            ...
            {
                if (bp -> Tag() == BinaryExpressionEnum::EQUAL_EQUAL)
        ...
        //
        if (left_type == lookupEnv.boolean_type &&
            (IsOne(left) || IsOne(right)))
        {
            EmitBranchIfExpression(IsOne(left) ? right : left,
                                   cond == (bp -> Tag() == BinaryExpressionEnum::EQUAL_EQUAL),
                                   lab, over);
            return;
        }
        ...
        //
        if (lookupEnv.IsSimpleIntegerValueType(left_type) ||
             left_type == lookupEnv.boolean_type)
        {
            assert(lookupEnv.IsSimpleIntegerValueType(right_type) ||
                   right_type == lookupEnv.boolean_type);
    ...
    switch (bp -> Tag())
    {
    case BinaryExpressionEnum::LESS:
    case BinaryExpressionEnum::LESS_EQUAL:
    case BinaryExpressionEnum::GREATER:
    case BinaryExpressionEnum::GREATER_EQUAL:
    case BinaryExpressionEnum::EQUAL_EQUAL:
    case BinaryExpressionEnum::NOT_EQUAL:
    ...
    else if (left_type == lookupEnv.long_type)
    {
        EmitExpression(left);
        EmitExpression(right);
    ...
    else if (left_type == lookupEnv.float_type)
    {
        EmitExpression(left);
        EmitExpression(right);
    ...
    else if (left_type == lookupEnv.double_type)
    {
        EmitExpression(left);
        EmitExpression(right);
        switch (bp -> Tag())
...
//
bool ByteCode::EmitSynchronizedStatement(SynchronizedStatement* statement)
{
    int variable_index =
    ...
    assert(stack_depth == 0);
    stack_depth = 1; // account for the exception already on the stack
    LoadLocal(variable_index, lookupEnv.Object()); // reload monitor
    PutOp(OP_MONITOREXIT);
    ...
    StoreLocal(variable_index, lookupEnv.Object()); // save address of object
...
void ByteCode::EmitAssertStatement(AssertStatement* assertion)
{
    //
    ...
    PutOp(OP_NEW);
    PutU2(RegisterClass(lookupEnv.AssertionError()));
    PutOp(OP_DUP);
    ...
    if (assertion -> message_opt)
    {
        ...
        if (! lookupEnv.AssertionError() -> Bad())
        {
            // We found the class, now can we find the method?
            if (type == lookupEnv.char_type)
                constructor = lookupEnv.AssertionError_InitWithCharMethod();
            else if (type == lookupEnv.boolean_type)
                constructor = lookupEnv.AssertionError_InitWithBooleanMethod();
            else if (type == lookupEnv.int_type || type == lookupEnv.short_type ||
                     type == lookupEnv.byte_type)
            {
                constructor = lookupEnv.AssertionError_InitWithIntMethod();
            }
            else if (type == lookupEnv.long_type)
                constructor = lookupEnv.AssertionError_InitWithLongMethod();
            else if (type == lookupEnv.float_type)
                constructor = lookupEnv.AssertionError_InitWithFloatMethod();
            else if (type == lookupEnv.double_type)
                constructor = lookupEnv.AssertionError_InitWithDoubleMethod();
            else if (type == lookupEnv.null_type || IsReferenceType(type))
                constructor = lookupEnv.AssertionError_InitWithObjectMethod();
    ...
    else constructor = lookupEnv.AssertionError_InitMethod();
...
void ByteCode::EmitForeachStatement(ForeachStatement* foreach)
{
    int helper_index =
        method_stack -> TopBlock() -> block_symbol -> helper_variable_index;
    bool abrupt;
    ...
    if (expr_type -> IsArray())
    {
        ...
        PutOp(OP_ARRAYLENGTH);
        PutOp(OP_DUP);
        StoreLocal(helper_index + 1, lookupEnv.int_type);
        EmitBranch(OP_IFEQ, end);
        PutOp(OP_ICONST_0);
        StoreLocal(helper_index + 2, lookupEnv.int_type);
        PutOp(OP_ICONST_0);
        DefineLabel(loop);
        ...
        if (! abrupt || foreach -> statement -> can_complete_normally)
        {
            ...
            LoadLocal(helper_index + 1, lookupEnv.int_type);
            LoadLocal(helper_index + 2, lookupEnv.int_type);
            PutOp(OP_DUP_X1);
    ...
    else
    {
        ...
        PutU2(RegisterLibraryMethodref(lookupEnv.Iterable_iteratorMethod()));
        PutU1(1);
        PutU1(0);
        ChangeStack(1);
        PutOp(OP_DUP);
        PutOp(OP_INVOKEINTERFACE);
        u2 hasNext_index =
            RegisterLibraryMethodref(lookupEnv.Iterator_hasNextMethod());
        PutU2(hasNext_index);
        PutU1(1);
        PutU1(0);
        ChangeStack(1);
        ...
        PutOp(OP_DUP);
        StoreLocal(helper_index, lookupEnv.Iterator());
        DefineLabel(loop);
        PutOp(OP_INVOKEINTERFACE);
        PutU2(RegisterLibraryMethodref(lookupEnv.Iterator_nextMethod()));
        PutU1(1);
        PutU1(0);
        ChangeStack(1);
        if (component_type != lookupEnv.Object())
        {
            PutOp(OP_CHECKCAST);
        ...
        {
            DefineLabel(comp);
            LoadLocal(helper_index, lookupEnv.Iterator());
            PutOp(OP_DUP);
            PutOp(OP_INVOKEINTERFACE);
            PutU2(hasNext_index);
            PutU1(1);
...
//
int ByteCode::EmitExpression(Expression* expression, bool need_value)
{
    ...
    case Ast::CLASS_LITERAL:
        {
            ClassLiteralAccess* class_lit = (ClassLiteralAccess*) expression;
            if (class_lit -> resolution_opt)
                return GenerateClassAccess(class_lit, need_value);
            TypeSymbol* type =  TypeSymbol::TypeCast(expression->symbol);
            if (type)
            {
                // Must load for side effect of class not found
                assert(type == lookupEnv.Class());
            ...
            else if (need_value)
            {
                ...
                assert(VariableSymbol::VariableCast(expression -> symbol) );
                PutOp(OP_GETSTATIC);
    ...
    case Ast::DOT:
        return EmitFieldAccess((FieldReference*) expression, need_value);
    case Ast::CALL:
    ...
    case Ast::ARRAY_ACCESS:
        {
            ...
            int words = EmitArrayAccessRhs((ArrayReference*) expression);
            if (need_value)
                return words;
    ...
    case Ast::PRE_UNARY:
        return EmitPreUnaryExpression((AstPreUnaryExpression*) expression,
                                      need_value);
    case Ast::CAST:
        return EmitCastExpression((CastExpression*) expression, need_value);
    case Ast::BINARY:
        return EmitBinaryExpression((BinaryExpression*) expression,
                                    need_value);
    case Ast::INSTANCEOF:
        return EmitInstanceofExpression((InstanceofExpression*) expression,
                                        need_value);
    case Ast::CONDITIONAL:
        return EmitConditionalExpression(((ConditionalExpression*)
                                          expression),
                                         need_value);
    case Ast::ASSIGNMENT:
...
Expression* ByteCode::VariableExpressionResolution(Expression* expression)
{
    //
    ...
    //
    expression = StripNops(expression);
    FieldReference* field = FieldReference::FieldAccessCast(expression);
...
TypeSymbol* ByteCode::VariableTypeResolution(Expression* expression,
                                             VariableSymbol* sym)
{
...
TypeSymbol* ByteCode::MethodTypeResolution(Expression* base,
                                           MethodSymbol* msym)
{
    //
    ...
    return owner_type == lookupEnv.Object() ? owner_type : base_type;
}


void ByteCode::EmitFieldAccessLhsBase(Expression* expression)
{
    expression = VariableExpressionResolution(expression);
    FieldReference* field = FieldReference::FieldAccessCast(expression);
    AstName* name = AstName::NameCast(expression);
...
void ByteCode::EmitFieldAccessLhs(Expression* expression)
{
    EmitFieldAccessLhsBase(expression);
    PutOp(OP_DUP);     // save base address of field for later store
    PutOp(OP_GETFIELD);
    if (lookupEnv.IsDoubleWordType(expression -> Type()))
        ChangeStack(1);
...
{
    assert(lookupEnv.option->target < JikesOption::SDK1_5);
    //
    ...
    PutU2(RegisterLibraryMethodref(lookupEnv.Class_forNameMethod()));
    PutOp(OP_ILOAD_1);
    EmitBranch(OP_IFNE, label);
    PutOp(OP_INVOKEVIRTUAL);
    PutU2(RegisterLibraryMethodref(lookupEnv.Class_getComponentTypeMethod()));
    ChangeStack(1); // account for the return
    DefineLabel(label);
    CompleteLabel(label);
    PutOp(OP_ARETURN);
    code_attribute ->
      AddException(0, 12, 12, RegisterClass(lookupEnv.ClassNotFoundException()));

    ChangeStack(1); // account for the exception on the stack
    if (lookupEnv.option->target < JikesOption::SDK1_4)
    {
        PutOp(OP_INVOKEVIRTUAL);
        PutU2(RegisterLibraryMethodref(lookupEnv.Throwable_getMessageMethod()));
        ChangeStack(1); // account for the returned string
        PutOp(OP_NEW);
        PutU2(RegisterClass(lookupEnv.NoClassDefFoundError()));
        PutOp(OP_DUP_X1);
        PutOp(OP_SWAP);
        PutOp(OP_INVOKESPECIAL);
        PutU2(RegisterLibraryMethodref(lookupEnv.NoClassDefFoundError_InitStringMethod()));
    ...
    else
    {
        ...
        PutOp(OP_DUP_X1);
        PutOp(OP_INVOKESPECIAL);
        PutU2(RegisterLibraryMethodref(lookupEnv.NoClassDefFoundError_InitMethod()));
        PutOp(OP_INVOKEVIRTUAL);
        PutU2(RegisterLibraryMethodref(lookupEnv.Throwable_initCauseMethod()));
    }
    PutOp(OP_ATHROW);
}
...
// ldc class).
//
int ByteCode::GenerateClassAccess(ClassLiteralAccess* class_lit,
                                  bool need_value)
{
    assert(lookupEnv.option->target < JikesOption::SDK1_5);
    //
    ...
    //
    Label label;
    assert(VariableSymbol::VariableCast( class_lit -> symbol) );
    ...
    if (type -> num_dimensions > 255)
        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW, class_lit);
    bool is_array = type -> IsArray();
    if (! is_array)
        type = type -> GetArrayType(lookupEnv.system_semantic, 1);
    LoadLiteral(type -> FindOrInsertClassLiteralName(lookupEnv),
                lookupEnv.String());
                                                 ...
                                                 VariableSymbol* vsym)
{
    ...
    assert(! lookupEnv.option->noassert &&
           lookupEnv.option->target >= JikesOption::SDK1_4);
    tsym = tsym -> GetArrayType(lookupEnv.system_semantic, 1);
    LoadLiteral(tsym -> FindOrInsertClassLiteralName(lookupEnv),
                lookupEnv.String());
    PutOp(OP_INVOKESTATIC);
    PutU2(RegisterLibraryMethodref(lookupEnv.Class_forNameMethod()));
    PutOp(OP_INVOKEVIRTUAL);
    ...
    PutOp(OP_INVOKEVIRTUAL);
    ChangeStack(1); // for returned value
    PutU2(RegisterLibraryMethodref(lookupEnv.Class_desiredAssertionStatusMethod()));
    PutOp(OP_ICONST_1);
    PutOp(OP_IXOR);
    PutOp(OP_PUTSTATIC);
...
{
    if (TypeSymbol::TypeCast(expression->symbol))
        return 0;
    VariableSymbol* var = VariableSymbol::VariableCast(expression -> symbol);
                                          ...
                                          bool need_value)
{
    ...
    if (expression -> Type() -> num_dimensions > 255)
        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW, expression);
    ...
    else
    {
        ...
        for (unsigned i = 0; ! create_array && i < num_dims; i++)
        {
            ...
            else if (expr -> Type() != lookupEnv.char_type)
                create_array = true;
        }
                                       ...
                                       bool need_value)
{
    ...
    // JLS2 added ability for parenthesized variable to remain a variable.
    //
    CastExpression* casted_left_hand_side =
        CastExpression::CastExpressionCast(assignment_expression->left_hand_side);
    ...
    VariableSymbol* accessed_member = assignment_expression -> write_method
        ? (VariableSymbol::VariableCast(assignment_expression -> write_method -> accessed_member) )
        : (VariableSymbol*) NULL;
    ...
    if (assignment_expression -> SimpleAssignment())
    {
        ...
        case ARRAY_VAR:
            ...
            EmitArrayAccessLhs( ArrayReference::ArrayAccessCast(left_hand_side));
            break;
        case FIELD_VAR:
        ...
        case STATIC_VAR:
            ...
            //
            if (FieldReference::FieldAccessCast(left_hand_side))
            {
                Expression* base =
                    ((FieldReference*) left_hand_side) -> base;
                EmitExpression(base, false);
            }
            else if (AstName::NameCast(left_hand_side))
            {
        ...
        case ACCESSED_VAR:
            ...
            {
                Expression* resolve = FieldReference::FieldAccessCast(left_hand_side)
                    ? FieldReference::FieldAccessCast(left_hand_side) -> resolution_opt
                    : AstName::NameCast(left_hand_side) -> resolution_opt;
                assert(resolve);

                Expression* base =
                    AstMethodInvocation::MethodInvocationCast(resolve) -> base_opt;
                assert(base);
                EmitExpression(base);
            }
            else if (FieldReference::FieldAccessCast(left_hand_side))
                //
                // If the access is qualified by an arbitrary base
                // expression, evaluate it for side effects.
                //
                EmitExpression(((FieldReference*) left_hand_side) -> base,
                               false);
            break;
        case LOCAL_VAR:
            break;
    ...
    else
    {
        ...
        if ((assignment_expression -> Tag() ==
			AssignmentExpressionEnum::PLUS_EQUAL) &&
           (lookupEnv.IsString(left_type)))
        {
            PutOp(OP_NEW);
            PutU2(RegisterClass(lookupEnv.option->target >= JikesOption::SDK1_5
                                ? lookupEnv.StringBuilder()
                                : lookupEnv.StringBuffer()));
            PutOp(OP_DUP_X1);
            PutOp(OP_INVOKESPECIAL);
            PutU2(RegisterLibraryMethodref
                  (lookupEnv.option->target >= JikesOption::SDK1_5
                   ? lookupEnv.StringBuilder_InitMethod()
                   : lookupEnv.StringBuffer_InitMethod()));
            EmitStringAppendMethod(lookupEnv.String());
            AppendString(assignment_expression -> expression, true);
            PutOp(OP_INVOKEVIRTUAL);
            PutU2(RegisterLibraryMethodref
                  (lookupEnv.option->target >= JikesOption::SDK1_5
                   ? lookupEnv.StringBuilder_toStringMethod()
                   : lookupEnv.StringBuffer_toStringMethod()));
        ...
        else
        {
            ...
            if (lookupEnv.IsSimpleIntegerValueType(op_type) ||
                op_type == lookupEnv.boolean_type)
            {
            ...
            else if (op_type == lookupEnv.long_type)
            {
                switch (assignment_expression -> Tag())
                {
                case AssignmentExpressionEnum::STAR_EQUAL:
            ...
            else if (op_type == lookupEnv.float_type)
            {
                switch (assignment_expression -> Tag())
                {
                case AssignmentExpressionEnum::STAR_EQUAL:
            ...
            else if (op_type == lookupEnv.double_type)
            {
                switch (assignment_expression -> Tag())
                {
                case AssignmentExpressionEnum::STAR_EQUAL:
    ...
    case ARRAY_VAR:
        if (need_value)
            PutOp(lookupEnv.IsDoubleWordType(left_type) ? OP_DUP2_X2 : OP_DUP_X2);
        StoreArrayElement(assignment_expression -> Type());
        break;
    case FIELD_VAR:
        if (need_value)
            PutOp(lookupEnv.IsDoubleWordType(left_type) ? OP_DUP2_X1 : OP_DUP_X1);
    ...
    case ACCESSED_VAR:
        {
            ...
            {
                if (accessed_member -> ACC_STATIC())
                    PutOp(lookupEnv.IsDoubleWordType(left_type)
                          ? OP_DUP2 : OP_DUP);
                else PutOp(lookupEnv.IsDoubleWordType(left_type)
    ...
    case STATIC_VAR:
        if (need_value)
            PutOp(lookupEnv.IsDoubleWordType(left_type) ? OP_DUP2 : OP_DUP);
...
//
int ByteCode::EmitBinaryExpression(BinaryExpression* expression,
                                   bool need_value)
{
    TypeSymbol* type = expression -> Type();
    ...
    //
    if (lookupEnv.IsString(type))
    {
        assert(expression -> Tag() == BinaryExpressionEnum::PLUS);
    ...
    if (! need_value)
    {
        if ((expression -> Tag() == BinaryExpressionEnum::SLASH ||
             expression -> Tag() == BinaryExpressionEnum::MOD) &&
            lookupEnv.IsIntegral(type) &&
             ...
             ! expression -> right_expression -> IsConstant()))
        {
            if (IsZero(expression -> right_expression))
            {
                ...
                //
                Expression* left_expr = expression -> left_expression;
                if ( CastExpression::CastExpressionCast(left_expr) &&
                    left_expr -> generated)
                {
                    left_expr = ((CastExpression*) left_expr) -> expression;
                }
                type = left_expr -> Type();
                EmitExpression(left_expr);
                PutOp(type == lookupEnv.long_type ? OP_LCONST_0 : OP_ICONST_0);
            }
            else
            {
            ...
            if (type == lookupEnv.long_type)
            {
                PutOp(expression -> Tag() == BinaryExpressionEnum::SLASH
                      ? OP_LDIV : OP_LREM);
                PutOp(OP_POP2);
            }
            else
        ...
        else if (expression -> Tag() == BinaryExpressionEnum::OR_OR)
        {
            //
            // if (cond || true); => cond;
            // if (cond || false); => cond;
        ...
        else if (expression -> Tag() == BinaryExpressionEnum::AND_AND)
        {
            //
            // if (cond && true); => cond;
            // if (cond && false); => cond;
    ...
    if (IsZero(expression -> left_expression))
    {
        ...
        //
        Expression* right_expr = expression -> right_expression;
        if ( CastExpression::CastExpressionCast(right_expr) && right_expr -> generated)
            right_expr = ((CastExpression*) right_expr) -> expression;
        ...
        case BinaryExpressionEnum::AND_AND:
            ...
            return 1;
        case BinaryExpressionEnum::EQUAL_EQUAL:
            if (right_type != lookupEnv.boolean_type)
                break;
        ...
        case BinaryExpressionEnum::STAR:
        case BinaryExpressionEnum::AND:
        case BinaryExpressionEnum::LEFT_SHIFT:
        case BinaryExpressionEnum::RIGHT_SHIFT:
        case BinaryExpressionEnum::UNSIGNED_RIGHT_SHIFT:
            //
            ...
            //
            if (lookupEnv.IsFloatingPoint(type))
                break;

            EmitExpression(right_expr, false);
            PutOp(type == lookupEnv.long_type ? OP_LCONST_0 : OP_ICONST_0);
            return GetTypeWords(type);
        case BinaryExpressionEnum::MINUS:
            //
            ...
            PutOp(lookupEnv.IsSimpleIntegerValueType(type) ? OP_INEG
                  : type == lookupEnv.long_type ? OP_LNEG
                  : type == lookupEnv.float_type ? OP_FNEG
                  : OP_DNEG); // double_type
            return GetTypeWords(type);
        default:
            break;
    ...
    {
        if (expression -> Tag() == BinaryExpressionEnum::STAR)
        {
            EmitExpression(expression -> right_expression);
            return GetTypeWords(type);
        }
        if (expression -> left_expression -> Type() == lookupEnv.boolean_type)
        {
            switch (expression -> Tag())
            {
            case BinaryExpressionEnum::EQUAL_EQUAL:
    ...
    if (IsZero(expression -> right_expression))
    {
        ...
        //
        Expression* left_expr = expression -> left_expression;
        if ( CastExpression::CastExpressionCast(left_expr) && left_expr -> generated)
            left_expr = ((CastExpression*) left_expr) -> expression;
        ...
        case BinaryExpressionEnum::UNSIGNED_RIGHT_SHIFT:
            ...
            //
            if (lookupEnv.IsFloatingPoint(left_type))
            {
                    ...
                    lookupEnv.float_type)
                {
                    ...
                    if (value -> value.IsPositiveZero() ==
                        (expression -> Tag() == BinaryExpressionEnum::PLUS))
                        break;
                }
    ...
    {
        if (expression -> Tag() == BinaryExpressionEnum::STAR ||
            expression -> Tag() == BinaryExpressionEnum::SLASH)
        {
            EmitExpression(expression -> left_expression);
            return GetTypeWords(type);
        }
        if (expression -> right_expression -> Type() == lookupEnv.boolean_type)
        {
            switch (expression -> Tag())
            {
            case BinaryExpressionEnum::EQUAL_EQUAL:
    ...
    //
    if (expression -> left_expression -> Type() == lookupEnv.boolean_type &&
        (expression -> Tag() == BinaryExpressionEnum::EQUAL_EQUAL ||
         expression -> Tag() == BinaryExpressionEnum::NOT_EQUAL))
    {
        EmitExpression(expression -> left_expression);
        EmitExpression(expression -> right_expression);
        PutOp(OP_IXOR);
        if (expression -> Tag() == BinaryExpressionEnum::EQUAL_EQUAL)
        {
            PutOp(OP_ICONST_1);
            PutOp(OP_IXOR);
        }
    ...
    bool integer_type = type == lookupEnv.boolean_type ||
        lookupEnv.IsSimpleIntegerValueType(type);
    switch (expression -> Tag())
    {
    case BinaryExpressionEnum::STAR:
        PutOp(integer_type ? OP_IMUL
              : type == lookupEnv.long_type ? OP_LMUL
              : type == lookupEnv.float_type ? OP_FMUL
              : OP_DMUL); // double_type
        break;
    case BinaryExpressionEnum::SLASH:
        PutOp(integer_type ? OP_IDIV
              : type == lookupEnv.long_type ? OP_LDIV
              : type == lookupEnv.float_type ? OP_FDIV
              : OP_DDIV); // double_type
        break;
    case BinaryExpressionEnum::MOD:
        PutOp(integer_type ? OP_IREM
              : type == lookupEnv.long_type ? OP_LREM
              : type == lookupEnv.float_type ? OP_FREM
              : OP_DREM); // double_type
        break;
    case BinaryExpressionEnum::PLUS:
        PutOp(integer_type ? OP_IADD
              : type == lookupEnv.long_type ? OP_LADD
              : type == lookupEnv.float_type ? OP_FADD
              : OP_DADD); // double_type
        break;
    case BinaryExpressionEnum::MINUS:
        PutOp(integer_type ? OP_ISUB
              : type == lookupEnv.long_type ? OP_LSUB
              : type == lookupEnv.float_type ? OP_FSUB
              : OP_DSUB); // double_type
        break;
    case BinaryExpressionEnum::LEFT_SHIFT:
...
int ByteCode::EmitInstanceofExpression(InstanceofExpression* expr,
                                       bool need_value)
{
    TypeSymbol* left_type = expr -> expression -> Type();
    TypeSymbol* right_type = expr -> type -> symbol;
    if (right_type -> num_dimensions > 255)
        semantic.ReportSemErrorWithAst(SemanticError::ARRAY_OVERFLOW, expr -> type);
    if (left_type == lookupEnv.null_type)
    {
        //
        // We know the result: false. But emit the left expression,
		...
		BinaryExpression::BinaryExpressionCast(expr->expression))
    {
        //
        // We know the result: true, since the string literals and string
        // concats are non-null and String is a final class.
        //
        assert( (left_type->getNameString() == L"String"/* &&¡¡left_type == lookupEnv.String()*/));
        EmitExpression(expr -> expression, false);
        if (need_value)
            PutOp(OP_ICONST_1);
    }
    else if (( AstThisExpression::ThisExpressionCast(expr->expression) ||
              AstSuperExpression::SuperExpressionCast(expr->expression) ||
              ClassLiteralAccess::ClassLiteralCast(expr->expression) ||
              AstClassCreationExpression::ClassCreationExpressionCast(expr->expression) ||
              AstArrayCreationExpression::ArrayCreationExpressionCast(expr->expression )) &&
             left_type -> IsSubtype(right_type))
    {
        //
        // We know the result: true, since the expression is non-null.
...
int ByteCode::EmitCastExpression(CastExpression* expression,
                                 bool need_value)
{
    ...
    if (need_value || dest_type -> IsSubtype(source_type))
    {
        ...
        if (! need_value)
        {
            assert(source_type -> IsSubtype(lookupEnv.Object()));
            PutOp(OP_POP);
        }
    }
...
void ByteCode::EmitCast(TypeSymbol* dest_type, TypeSymbol* source_type)
{
    if (source_type -> IsSubtype(dest_type) ||
        source_type == lookupEnv.null_type)
    {
        return; // done if nothing to do
    }

    if (lookupEnv.IsSimpleIntegerValueType(source_type))
    {
        if (dest_type == lookupEnv.int_type ||
            (source_type == lookupEnv.byte_type &&
             dest_type == lookupEnv.short_type))
        {
            return; // no conversion needed
        }
        Opcode op_kind = (dest_type == lookupEnv.long_type ? OP_I2L
                          : dest_type == lookupEnv.float_type ? OP_I2F
                          : dest_type == lookupEnv.double_type ? OP_I2D
                          : dest_type == lookupEnv.char_type ? OP_I2C
                          : dest_type == lookupEnv.byte_type ? OP_I2B
                          : OP_I2S); // short_type
        ...
        assert(op_kind != OP_I2S || dest_type == lookupEnv.short_type);

        PutOp(op_kind);
    }
    else if (source_type == lookupEnv.long_type)
    {
        Opcode op_kind = (dest_type == lookupEnv.float_type ? OP_L2F
                          : dest_type == lookupEnv.double_type ? OP_L2D
                          : OP_L2I);
        PutOp(op_kind);

        if (op_kind == OP_L2I && dest_type != lookupEnv.int_type)
        {
            assert(lookupEnv.IsSimpleIntegerValueType(dest_type) &&
                   "unsupported conversion");

            PutOp(dest_type == lookupEnv.char_type ? OP_I2C
                  : dest_type == lookupEnv.byte_type ? OP_I2B
                  : OP_I2S); // short_type
        }
    }
    else if (source_type == lookupEnv.float_type)
    {
        Opcode op_kind = (dest_type == lookupEnv.long_type ? OP_F2L
                          : dest_type == lookupEnv.double_type ? OP_F2D
                          : OP_F2I);
        PutOp(op_kind);

        if (op_kind == OP_F2I && dest_type != lookupEnv.int_type)
        {
            assert(lookupEnv.IsSimpleIntegerValueType(dest_type) &&
                   "unsupported conversion");
    ...
    else if (source_type == lookupEnv.double_type)
    {
        Opcode op_kind = (dest_type == lookupEnv.long_type ? OP_D2L
                          : dest_type == lookupEnv.float_type ? OP_D2F
                          : OP_D2I);

        PutOp(op_kind);

        if (op_kind == OP_D2I && dest_type != lookupEnv.int_type)
        {
            assert(lookupEnv.IsSimpleIntegerValueType(dest_type) &&
                   "unsupported conversion");
...
//
void ByteCode::EmitCheckForNull(Expression* expression, bool need_value)
{
    expression = StripNops(expression);

    if (expression -> Type() == lookupEnv.null_type)
    {
        //
	...
	VariableSymbol* variable = VariableSymbol::VariableCast(expression->symbol);
    if ( AstClassCreationExpression::ClassCreationExpressionCast(expression) ||
        AstThisExpression::ThisExpressionCast(expression) ||
        AstSuperExpression::SuperExpressionCast(expression) ||
        ClassLiteralAccess::ClassLiteralCast(expression) ||
        (variable && variable -> ACC_SYNTHETIC() &&
         variable -> Identity() == lookupEnv.nameEnv->this_name_symbol))
    {
    ...
    PutOp(OP_INVOKEVIRTUAL);
    ChangeStack(1); // for returned value
    PutU2(RegisterLibraryMethodref(lookupEnv.Object_getClassMethod()));
    PutOp(OP_POP);
}
...
int ByteCode::EmitConditionalExpression(ConditionalExpression* expression,
                                        bool need_value)
{
    //
    ...
    if (expression -> Type() == lookupEnv.null_type)
    {
        ...
        //
        if (NullLiteral::NullLiteralCast(expression->false_expression))
        {
            if (NullLiteral::NullLiteralCast(expression->true_expression))
    ...
    else if (expression -> true_expression -> IsConstant())
    {
        if (expression -> false_expression -> IsConstant())
        {
            ...
            if (lookupEnv.IsSimpleIntegerValueType(expression -> Type()) ||
                expression -> Type() == lookupEnv.boolean_type)
            {
                //
        ...
        else if ((lookupEnv.IsSimpleIntegerValueType(expression -> Type()) ||
                  expression -> Type() == lookupEnv.boolean_type) &&
...
int ByteCode::EmitFieldAccess(FieldReference* expression, bool need_value)
{
    if (expression -> resolution_opt)
        return LoadVariable(ACCESSED_VAR, expression, need_value);
    VariableSymbol* sym = VariableSymbol::VariableCast(expression -> symbol);
    assert(sym);
                                   ...
                                   bool need_value)
{
    ...
    AstMethodInvocation* method_call = expression -> resolution_opt
        ? AstMethodInvocation::MethodInvocationCast(expression->resolution_opt) : expression;
    assert(method_call);
    ...
    else
    {
        if (base)
        {
            ...
            //
            is_super = AstSuperExpression::SuperExpressionCast(base) != NULL;
            EmitExpression(base);
        }
                           ...
                           bool need_value, TypeSymbol* base_type)
{
    ...
    //
    if (msym -> Type() == lookupEnv.void_type)
        return 0;
    bool wide = lookupEnv.IsDoubleWordType(msym -> Type());
...
//
Expression* ByteCode::StripNops(Expression* expr)
{
    while (! expr -> IsConstant())
    {
        if ( AstParenthesizedExpression::ParenthesizedExpressionCast(expr))
            expr = ((AstParenthesizedExpression*) expr) -> expression;
        else if ( CastExpression::CastExpressionCast(expr))
        {
            CastExpression* cast_expr = (CastExpression*) expr;
            TypeSymbol* cast_type = expr -> Type();
            Expression* sub_expr = StripNops(cast_expr -> expression);
            TypeSymbol* sub_type = sub_expr -> Type();
            if (sub_type -> IsSubtype(cast_type) ||
                (sub_type == lookupEnv.byte_type &&
                 (cast_type == lookupEnv.short_type ||
                  cast_type == lookupEnv.int_type)) ||
                ((sub_type == lookupEnv.short_type ||
                  sub_type == lookupEnv.char_type) &&
                 cast_type == lookupEnv.int_type) ||
                (sub_type == lookupEnv.null_type &&
...
bool ByteCode::IsNop(AstBlock* block)
{
    ...
    {
        Ast* statement = block -> GetStatement(i);
        if (EmptyStatement::EmptyStatementCast(statement) ||
            AstLocalClassStatement::LocalClassStatementCast(statement) ||
            (AstBlock::BlockCast(statement) && IsNop((AstBlock*) statement)))
            continue;
...
void ByteCode::EmitNewArray(unsigned num_dims, const TypeSymbol* type)
{
    ...
    if (num_dims == 1)
    {
        ...
        if (lookupEnv.IsPrimitive(element_type))
        {
            PutOp(OP_NEWARRAY);
            PutU1(element_type == lookupEnv.boolean_type ? 4
                  : element_type == lookupEnv.char_type ? 5
                  : element_type == lookupEnv.float_type ? 6
                  : element_type == lookupEnv.double_type ? 7
                  : element_type == lookupEnv.byte_type ? 8
                  : element_type == lookupEnv.short_type ? 9
                  : element_type == lookupEnv.int_type ? 10
...
//
void ByteCode::EmitArrayAccessLhs(ArrayReference* expression)
{
    TypeSymbol* base_type = expression -> base -> Type();
    Expression* base = StripNops(expression -> base);
    EmitExpression(base);
    if (lookupEnv.option->target < JikesOption::SDK1_5 &&
        IsMultiDimensionalArray(base_type) &&
        base -> Type() == lookupEnv.null_type)
    {
        //
        // Prior to JDK 1.5, VMs incorrectly complained if assigning an array
        // type into an element of a null expression (in other words, null
    ...
    case ACCESSED_VAR:
        {
			VariableSymbol* accessed_member =
				VariableSymbol::VariableCast(expression->write_method->accessed_member);
            if (accessed_member -> ACC_STATIC())
                EmitPostUnaryExpressionSimple(kind, expression, need_value);
                                            ...
                                            bool need_value)
{
    ...
    if (need_value)
        PutOp(lookupEnv.IsDoubleWordType(expression_type)
              ? OP_DUP2_X1 : OP_DUP_X1);

    if (lookupEnv.IsSimpleIntegerValueType(expression_type))
    {
        PutOp(OP_ICONST_1);
        PutOp(expression -> Tag() == PostUnaryExpressionEnum::PLUSPLUS
              ? OP_IADD : OP_ISUB);
        EmitCast(expression_type, lookupEnv.int_type);
    }
    else if (expression_type == lookupEnv.long_type)
    {
        PutOp(OP_LCONST_1);
        PutOp(expression -> Tag() == PostUnaryExpressionEnum::PLUSPLUS
              ? OP_LADD : OP_LSUB);
    }
    else if (expression_type == lookupEnv.float_type)
    {
        PutOp(OP_FCONST_1);
        PutOp(expression -> Tag() == PostUnaryExpressionEnum::PLUSPLUS
              ? OP_FADD : OP_FSUB);
    }
    else if (expression_type == lookupEnv.double_type)
    {
    ...
    {
        PutOp(OP_PUTFIELD);
        if (lookupEnv.IsDoubleWordType(expression_type))
            ChangeStack(-1);
...
{
    TypeSymbol* expression_type = expression -> Type();
    if (kind == LOCAL_VAR && expression_type == lookupEnv.int_type)
    {
        // can we use IINC ??
        LoadVariable(kind, StripNops(expression -> expression), need_value); 
        PutOpIINC(VariableSymbol::VariableCast(expression -> symbol) -> LocalVariableIndex(),
                  expression -> Tag() == PostUnaryExpressionEnum::PLUSPLUS ? 1 : -1);
        return;
    }
    ...
    if (need_value)
        PutOp(lookupEnv.IsDoubleWordType(expression_type) ? OP_DUP2 : OP_DUP);
                                            ...
                                            bool need_value)
{
    ...
    // JLS2 added ability for parenthesized variable to remain a variable.
    //
    EmitArrayAccessLhs((ArrayReference*) StripNops(expression -> expression));
    // lhs must be array access
    PutOp(OP_DUP2); // save array base and index for later store

    TypeSymbol* expression_type = expression -> Type();
    if (expression_type == lookupEnv.int_type)
    {
         PutOp(OP_IALOAD);
    ...
    else if (expression_type == lookupEnv.byte_type )
    {
         PutOp(OP_BALOAD);
         if (need_value) // save value below saved array base and index
             PutOp(OP_DUP_X2);
    ...
    else if (expression_type == lookupEnv.char_type )
    {
         PutOp(OP_CALOAD);
         if (need_value) // save value below saved array base and index
             PutOp(OP_DUP_X2);
    ...
    else if (expression_type == lookupEnv.short_type)
    {
         PutOp(OP_SALOAD);
         if (need_value) // save value below saved array base and index
             PutOp(OP_DUP_X2);
...
{
    TypeSymbol* type = expression -> Type();
    if (expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS ||
        expression -> Tag() == PreUnaryExpressionEnum::MINUSMINUS)
    {
    ...
    else // here for ordinary unary operator without side effects.
    {
        ...
        switch (expression -> Tag())
        {
        case PreUnaryExpressionEnum::PLUS:
            // Nothing else to do.
            break;
        case PreUnaryExpressionEnum::MINUS:
            assert(lookupEnv.IsNumeric(type) && "unary minus on bad type");
            PutOp(lookupEnv.IsSimpleIntegerValueType(type) ? OP_INEG
                  : type == lookupEnv.long_type ? OP_LNEG
                  : type == lookupEnv.float_type ? OP_FNEG
                  : OP_DNEG); // double_type
            break;
        case PreUnaryExpressionEnum::TWIDDLE:
            ...
            break;
        case PreUnaryExpressionEnum::NOT:
            assert(type == lookupEnv.boolean_type);
            PutOp(OP_ICONST_1);
...
{
    TypeSymbol* type = expression -> Type();
    if (kind == LOCAL_VAR && type == lookupEnv.int_type)
    {
        PutOpIINC(VariableSymbol::VariableCast(expression -> symbol ) -> LocalVariableIndex(),
                  expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS ? 1 : -1);
        LoadVariable(kind, StripNops(expression -> expression), need_value);
        return;
    }
    ...
    {
        PutOp(OP_ICONST_1);
        PutOp(expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS
              ? OP_IADD : OP_ISUB);
        EmitCast(type, lookupEnv.int_type);
        if (need_value)
            PutOp(OP_DUP);
    }
    else if (type == lookupEnv.long_type)
                                                    ...
                                                    bool need_value)
{
    ...
    else if (type == lookupEnv.byte_type)
    {
         PutOp(OP_BALOAD);
         PutOp(OP_ICONST_1);
         PutOp(expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS
    ...
    else if (type == lookupEnv.char_type)
    {
         PutOp(OP_CALOAD);
         PutOp(OP_ICONST_1);
         PutOp(expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS
    ...
    else if (type == lookupEnv.short_type)
    {
         PutOp(OP_SALOAD);
         PutOp(OP_ICONST_1);
         PutOp(expression -> Tag() == PreUnaryExpressionEnum::PLUSPLUS
...
//
void ByteCode::ConcatenateString(BinaryExpression* expression,
                                 bool need_value)
{
    //
    ...
    //
    Expression* left_expr = StripNops(expression -> left_expression);
    if ( lookupEnv.IsString(left_expr->Type() ) &&
        BinaryExpression::BinaryExpressionCast(left_expr) &&
        ! left_expr -> IsConstant())
    {
        ConcatenateString((BinaryExpression*) left_expr, need_value);
    }
    else
    {
        PutOp(OP_NEW);
        ...
        if (left_expr -> IsConstant())
        {
            ...
            else
            {
                ...
                PutU2(RegisterLibraryMethodref
                      (lookupEnv.option->target >= JikesOption::SDK1_5
                       ? lookupEnv.StringBuilder_InitWithStringMethod()
                       : lookupEnv.StringBuffer_InitWithStringMethod()));
...
void ByteCode::AppendString(Expression* expression, bool need_value)
{
    //
    ...
    if (expression -> IsConstant())
    {
        ...
        assert(value != NULL);
        assert(! lookupEnv.IsPrimitive(type)); // Bug 2919.
    ...
    else
    {
        BinaryExpression* binary_expression =
            BinaryExpression::BinaryExpressionCast(expression);
        if (binary_expression && lookupEnv.IsString(type))
        {
            assert(binary_expression -> Tag() == BinaryExpressionEnum::PLUS);
            AppendString(binary_expression -> left_expression, need_value);
            AppendString(binary_expression -> right_expression, need_value);
            return;
        }
        if (! need_value && lookupEnv.IsPrimitive(type))
        {
...
void ByteCode::EmitStringAppendMethod(TypeSymbol* type)
{
    ...
    if (lookupEnv.option->target >= JikesOption::SDK1_5)
    {
        append_method =
            (type == lookupEnv.char_type
             ? lookupEnv.StringBuilder_append_charMethod()
             : type == lookupEnv.boolean_type
             ? lookupEnv.StringBuilder_append_booleanMethod()
             : (type == lookupEnv.int_type || type == lookupEnv.short_type ||
                type == lookupEnv.byte_type)
             ? lookupEnv.StringBuilder_append_intMethod()
             : type == lookupEnv.long_type
             ? lookupEnv.StringBuilder_append_longMethod()
             : type == lookupEnv.float_type
             ? lookupEnv.StringBuilder_append_floatMethod()
             : type == lookupEnv.double_type
             ? lookupEnv.StringBuilder_append_doubleMethod()
             : ( lookupEnv.IsString(type) || type == lookupEnv.null_type)
             ? lookupEnv.StringBuilder_append_stringMethod()
             : IsReferenceType(type)
             ? lookupEnv.StringBuilder_append_objectMethod()
    ...
    else
    {
        append_method =
            (type == lookupEnv.char_type
             ? lookupEnv.StringBuffer_append_charMethod()
             : type == lookupEnv.boolean_type
             ? lookupEnv.StringBuffer_append_booleanMethod()
             : (type == lookupEnv.int_type || type == lookupEnv.short_type ||
                type == lookupEnv.byte_type)
             ? lookupEnv.StringBuffer_append_intMethod()
             : type == lookupEnv.long_type
             ? lookupEnv.StringBuffer_append_longMethod()
             : type == lookupEnv.float_type
             ? lookupEnv.StringBuffer_append_floatMethod()
             : type == lookupEnv.double_type
             ? lookupEnv.StringBuffer_append_doubleMethod()
             : ( lookupEnv.IsString(type) || type == lookupEnv.null_type)
             ? lookupEnv.StringBuffer_append_stringMethod()
             : IsReferenceType(type)
             ? lookupEnv.StringBuffer_append_objectMethod()
             : (MethodSymbol*) NULL); // for assertion
    }
    assert(append_method &&
           "unable to find method for string buffer concatenation");
    PutOp(OP_INVOKEVIRTUAL);
    if (lookupEnv.IsDoubleWordType(type))
        ChangeStack(-1);
...
ByteCode::ByteCode(TypeSymbol* type)
    : ClassFile()
    , lookupEnv(type -> semantic_environment -> sem -> lookup_env)
...
{
#ifdef JIKES_DEBUG
    if (! lookupEnv.option->nowrite)
        lookupEnv.class_files_written++;
#endif // JIKES_DEBUG
    ...
    switch (lookupEnv.option->target)
    {
...
void ByteCode::DefineLabel(Label& lab)
{
    ...
    if (last_op_goto && index >= 0 && ! (lookupEnv.option->g & JikesOption::VARS))
    {
...
{
    if (lookupEnv.IsSimpleIntegerValueType(type) || type == lookupEnv.boolean_type)
    {
         if (varno <= 3)
...
void ByteCode::LoadLiteral(LiteralValue* litp, const TypeSymbol* type)
{
    ...
    else if ( lookupEnv.IsString(type) || type == lookupEnv.null_type)
    {
...
void ByteCode::LoadImmediateInteger(i4 val)
{
    ...
    else if (val >= -32768 && val < 32768)
    {
        ...
        //
        u2 index = FindInteger(lookupEnv.nameEnv->int_pool.Find(val));
    ...
    else LoadConstantAtIndex(RegisterInteger(lookupEnv.nameEnv->int_pool.FindOrInsert(val)));
}
...
// or "op=".
//
void ByteCode::ResolveAccess(Expression* p)
{
    //
    // JLS2 added ability for parenthesized variable to remain a variable.
    //
    ...
    FieldReference* field = FieldReference::FieldAccessCast(p);
    Expression* resolve_expression = field ? field -> resolution_opt
        : AstName::NameCast(p) -> resolution_opt;
    AstMethodInvocation* read_method =
        AstMethodInvocation::MethodInvocationCast(resolve_expression);
    ...
    PutOp(OP_DUP);
    PutOp(OP_INVOKESTATIC);
    CompleteCall(MethodSymbol::MethodCast(read_method -> symbol), stack_words);
}


int ByteCode::LoadVariable(VariableCategory kind, Expression* expr,
                           bool need_value)
{
    ...
    case ACCESSED_VAR:
        {
            ...
            //
            MethodSymbol* method = MethodSymbol::MethodCast(expr -> symbol);
    ...
    case FIELD_VAR:
        ...
        if (shadow_parameter_offset && sym -> owner == unit_type &&
            (sym -> accessed_local ||
             sym -> Identity() == lookupEnv.nameEnv->this_name_symbol))
        {
            //
        ...
        {
            assert(sym -> name_symbol == lookupEnv.nameEnv->length_name_symbol);
            if (AstArrayCreationExpression::ArrayCreationExpressionCast(base) && ! need_value)
            {
                EmitExpression(base, false);
                return 0;
            }
    ...
    PutOp(lookupEnv.IsDoubleWordType(expression_type) ? OP_POP2 : OP_POP);
    return 0;
}
...
{
    PutOp((type == lookupEnv.byte_type ||
           type == lookupEnv.boolean_type) ? OP_BALOAD
          : type == lookupEnv.short_type ? OP_SALOAD
          : type == lookupEnv.int_type ? OP_IALOAD
          : type == lookupEnv.long_type ? OP_LALOAD
          : type == lookupEnv.char_type ? OP_CALOAD
          : type == lookupEnv.float_type ? OP_FALOAD
          : type == lookupEnv.double_type ? OP_DALOAD
...
{
    PutOp((type == lookupEnv.byte_type ||
           type == lookupEnv.boolean_type) ? OP_BASTORE
          : type == lookupEnv.short_type ? OP_SASTORE
          : type == lookupEnv.int_type ? OP_IASTORE
          : type == lookupEnv.long_type ? OP_LASTORE
          : type == lookupEnv.char_type ? OP_CASTORE
          : type == lookupEnv.float_type ? OP_FASTORE
          : type == lookupEnv.double_type ? OP_DASTORE
...
//
void ByteCode::StoreField(Expression* expression)
{
    VariableSymbol* sym = (VariableSymbol*) expression -> symbol;
    TypeSymbol* expression_type = expression -> Type();
    if (sym -> ACC_STATIC())
...
void ByteCode::StoreVariable(VariableCategory kind, Expression* expr)
{
    VariableSymbol* sym = (VariableSymbol*) expr -> symbol;
    switch (kind)
    {
...
void ByteCode::FinishCode()
{
    ...
    //
    if (lookupEnv.option->g & JikesOption::SOURCE)
        AddAttribute(new SourceFileAttribute
                     (RegisterUtf8(lookupEnv.nameEnv->SourceFile_literal),
    ...
    if (MethodSymbol::MethodCast(unit_type -> owner))
    {
...
{
#ifdef JIKES_DEBUG
    if (lookupEnv.option->debug_trap_op &&
        code_attribute -> CodeLength() == (u2) lookupEnv.option->debug_trap_op)
    {
        op_trap();
    }
