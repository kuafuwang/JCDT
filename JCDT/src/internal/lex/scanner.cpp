...
#include "JCDT_Lib/internal/parser/LPG/javadef.h"
...
//
Scanner::Scanner(Option* option_, INameEnvironment* name_env)
    : option(option_),
	  nameEnv(name_env),
...
{
    //
    // If this assertion fails, the GetToken structure in LexStream.h must be
    // redesigned !!!
    //
...
#ifdef JIKES_DEBUG
    if (option->debug_comments)
    {
        // Add 0th comment.
        Comment* current_comment = lex->NewComent();
...
//
void Scanner::SetUp(FileBase* file_symbol)
{
	file_symbol->lex_stream = new LexStream(option, file_symbol);
	lex = file_symbol->lex_stream;
    Initialize();
    lex -> CompressSpace();
...
// input is the name of a regular text file. Its output is a stream of tokens.
//
void Scanner::Scan(FileBase* file_symbol)
{
	auto temp = new LexStream(option, file_symbol);
	lex = temp;
    Initialize();
	lex-> ReadInput();
    ...
    file_symbol -> lex_stream = temp;
}

void Scanner::Scan(BaseLexStream* _lex)
{
	lex = _lex;
	Initialize();
	lex->RereadInput();
...
void Scanner::Scan()
{
    ...
    //
	vector<Token*>& token_stream = lex->token_stream;
    do
    {
        //
        ...
        if (! current_token_index || current_token -> Kind())
        {
            ...
            current_token =  token_stream[current_token_index];
        }
        else
        {
...
void Scanner::ScanStarComment()
{
    ...
    current_token -> SetKind(0);
#ifdef JIKES_DEBUG
    Comment* current_comment = NULL;
    if (option->debug_comments)
    {
        current_comment = lex->NewComent();
        current_comment -> string = NULL;
...
void Scanner::ScanSlashComment()
{
    ...
    if (option->debug_comments)
    {
        ...
        current_comment -> string = NULL;
        current_comment -> previous_token = current_token_index - 1;
        current_comment -> location = current_token -> start_location;
...
void Scanner::ClassifyCharLiteral()
{
    ...
    case U_SINGLE_QUOTE:
        ...
        if (ptr[1] == U_SINGLE_QUOTE)
        {
            lex -> ReportMessage(StreamError::ESCAPE_EXPECTED,
                                 current_token -> start_location + 1,
                                 current_token -> start_location + 1);
        }
        else
        {
    ...
    case U_BACKSLASH:
        ...
        default:
            lex -> ReportMessage(StreamError::INVALID_ESCAPE_SEQUENCE,
                                 current_token -> start_location + 1,
                                 current_token -> start_location + ptr - cursor);
            bad = true;
        }
        break;
    case U_CARRIAGE_RETURN:
    ...
    ptr++;
    current_token ->
        SetSymbol(nameEnv->char_table.FindOrInsertLiteral(cursor,
                                                         ptr - cursor));
    cursor = ptr;
}
...
void Scanner::ClassifyStringLiteral()
{
    ...
    ptr++;
    current_token ->
        SetSymbol(nameEnv->string_table.FindOrInsertLiteral(cursor,
                                                           ptr - cursor));
    cursor = ptr;
}
...
void Scanner::ClassifyIdOrKeyword()
{
        ...
        option->source < JikesOption::SDK1_4)
    {
        lex -> ReportMessage(StreamError::DEPRECATED_IDENTIFIER_ASSERT,
                             current_token -> start_location,
                             current_token -> start_location + len - 1);
        current_token -> SetKind(TK_Identifier);
    }
    if (current_token -> Kind() == TK_enum &&
        option->source < JikesOption::SDK1_5)
    {
    ...
    {
        current_token -> SetNameSymbol(nameEnv->FindOrInsertName(cursor, len));
        for (unsigned i = 0; i < option->keyword_map.Length(); i++)
        {
            if (option->keyword_map[i].length == len &&
                wcsncmp(cursor, option->keyword_map[i].name, len) == 0)
            {
                current_token -> SetKind(option->keyword_map[i].key);
            }
        }
    }
  ...
  //      //
		//auto stack_size = lbrace_stack.size();
  //     
		//
  //  }
...
void Scanner::ClassifyNumericLiteral()
{
    ...
    if (*ptr == U_f || *ptr == U_F)
    {
        len = ++ptr - cursor;
        current_token ->
            SetSymbol(nameEnv->float_table.FindOrInsertLiteral(cursor, len));
    ...
    else if (*ptr == U_d || *ptr == U_D)
    {
        len = ++ptr - cursor;
        current_token ->
            SetSymbol(nameEnv->double_table.FindOrInsertLiteral(cursor, len));
        current_token -> SetKind(TK_DoubleLiteral);
    }
    else if (current_token -> Kind() == TK_IntegerLiteral)
    {
        if (*ptr == U_l || *ptr == U_L)
        {
            ...
            len = ++ptr - cursor;
            current_token ->
                SetSymbol(nameEnv->long_table.FindOrInsertLiteral(cursor, len));
        ...
        else
        {
            len = ptr - cursor;
            current_token ->
                SetSymbol(nameEnv->int_table.FindOrInsertLiteral(cursor, len));
        }
    }
    else
    {
...
void Scanner::ClassifyLbrace()
{
    ...
    //
    lbrace_stack.push( current_token_index );
...
{
    //
    // When a left brace in encountered, it is pushed into the lbrace_stack.
...
void Scanner::ClassifyBadToken()
{
    ...
    current_token -> SetKind(0);
    lex -> ReportMessage(StreamError::BAD_TOKEN, current_token -> start_location,
                         cursor - lex -> InputBuffer() - 1);
}
