...
//
void Semantic::DefiniteExpression(Expression* expr, DefinitePair& def_pair)
{
...
//
DefiniteAssignmentSet* Semantic::DefiniteBooleanExpression(Expression* expr,
                                                           DefinitePair& def_pair)
{
    ...
    else if (expr -> symbol != lookup_env.no_type)
        definite = (this ->* DefiniteExpr[expr -> kind])(expr, def_pair);

    assert(! definite || expr -> Type() == lookup_env.boolean_type);
    return definite;
}


DefiniteAssignmentSet* Semantic::DefiniteName(Expression* expression,
                                              DefinitePair& def_pair)
{
    ...
    VariableSymbol* variable = name -> symbol
        ? VariableSymbol::VariableCast(name -> symbol) : (VariableSymbol*) NULL;
        ...
        (variable -> IsLocal(ThisMethod()) || variable -> IsFinal(ThisType())))
    {
        ...
        {
            ReportSemErrorSimple(SemanticError::VARIABLE_NOT_DEFINITELY_ASSIGNED,
...
DefiniteAssignmentSet* Semantic::DefiniteArrayAccess(Expression* expression,
                                                     DefinitePair& def_pair)
{
    ArrayReference* array_access = (ArrayReference*) expression;
...
DefiniteAssignmentSet* Semantic::DefiniteMethodInvocation(Expression* expression,
                                                          DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteClassCreationExpression(Expression* expression,
                                                                 DefinitePair& def_pair)
{
    unsigned i;
...
DefiniteAssignmentSet* Semantic::DefiniteArrayCreationExpression(Expression* expression,
                                                                 DefinitePair& def_pair)
{
...
inline VariableSymbol* Semantic::DefiniteFinal(FieldReference* field_access)
{
    if (field_access -> resolution_opt)
        field_access = FieldReference::FieldAccessCast(field_access->resolution_opt);

    if (field_access)
    {
        VariableSymbol* variable = (field_access -> symbol
                                    ? VariableSymbol::VariableCast(field_access -> symbol)
                                    : (VariableSymbol*) NULL);
        if (variable && variable -> IsFinal(ThisType()))
        {
            ...
            if (variable -> ACC_STATIC() ||
               AstThisExpression::ThisExpressionCast(field_access->base))
            {
                return variable;
            }
        }
...
DefiniteAssignmentSet* Semantic::DefinitePLUSPLUSOrMINUSMINUS(Expression* expr,
                                                              DefinitePair& def_pair)
{
    ...
    //
    while ( AstParenthesizedExpression::ParenthesizedExpressionCast(expr))
        expr = ((AstParenthesizedExpression*) expr) -> expression;

    VariableSymbol* variable = NULL;
    if (! ArrayReference::ArrayAccessCast(expr)) // some kind of name
    {
        ...
        {
            if (name -> resolution_opt)
                read_method = MethodSymbol::MethodCast(name -> resolution_opt -> symbol);
        }
        else
        {
		...
		variable = (read_method
			...
			: VariableSymbol::VariableCast(expr->symbol) );
...
DefiniteAssignmentSet* Semantic::DefinitePostUnaryExpression(Expression* expression,
                                                             DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteNOT(Expression* expr,
                                             DefinitePair& def_pair)
{
...
//
DefiniteAssignmentSet* Semantic::DefiniteDefaultPreUnaryExpression(Expression* expr,
                                                                   DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefinitePreUnaryExpression(Expression* expression,
                                                            DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteAND_AND(BinaryExpression* expr,
                                                 DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteOR_OR(BinaryExpression* expr,
                                               DefinitePair& def_pair)
{
    DefiniteAssignmentSet* after_left =
        DefiniteBooleanExpression(expr -> left_expression, def_pair);
...
DefiniteAssignmentSet* Semantic::DefiniteDefaultBinaryExpression(BinaryExpression* expr,
                                                                 DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteBinaryExpression(Expression* expression,
                                                          DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteInstanceofExpression(Expression* expression,
                                                              DefinitePair& def_pair)
{
    InstanceofExpression* expr = (InstanceofExpression*) expression;
    DefiniteExpression(expr -> expression, def_pair);
    return NULL;
}


DefiniteAssignmentSet* Semantic::DefiniteConditionalExpression(Expression* expression,
                                                               DefinitePair& def_pair)
{
    ...
    if (conditional_expression -> Type() == lookup_env.boolean_type)
    {
        if (! after_true)
...
DefiniteAssignmentSet* Semantic::DefiniteAssignmentExpression(Expression* expression,
                                                              DefinitePair& def_pair)
{
    ...
    else
    {
        ...
        if (field_access)
        {
            ...
            //
            if (AstThisExpression::ThisExpressionCast(field_access->base))
    ...
    VariableSymbol* variable = (left_hand_side -> symbol
                                ? VariableSymbol::VariableCast(left_hand_side -> symbol )
                                : (VariableSymbol*) NULL);
    while (variable && variable -> accessed_local)
        variable = variable -> accessed_local;
    int index = 0;
...
DefiniteAssignmentSet* Semantic::DefiniteParenthesizedExpression(Expression* expression,
                                                                 DefinitePair& def_pair)
{
...
DefiniteAssignmentSet* Semantic::DefiniteFieldAccess(Expression* expression,
                                                     DefinitePair& def_pair)
{
    FieldReference* expr = (FieldReference*) expression;
...
DefiniteAssignmentSet* Semantic::DefiniteCastExpression(Expression* expression,
                                                        DefinitePair& def_pair)
{
                                        ...
                                        DefinitePair& def_pair)
{
         ...
         i < array_initializer -> NumVariableInitializers(); i++)
    {
        ...
        {
            Expression* init =
                (Expression*) array_initializer -> VariableInitializer(i);
...
inline void Semantic::DefiniteVariableInitializer(AstVariableDeclarator* variable_declarator)
{
    ...
    Expression* init =
        Expression::ExpressionCast(variable_declarator->variable_initializer_opt);
    if (! init)
...
inline void Semantic::DefiniteBlockStatements(AstBlock* block_body)
{
    ...
    {
        ReachComleteAbleStatement* statement = block_body -> GetStatement(i);
        //
...
void Semantic::DefiniteLocalClassStatement(Ast* stmt)
{
    ...
    for (unsigned i = 0; i < local_type -> NumConstructorParameters(); i++)
    {
        ...
        {
            ReportSemErrorWithAst(SemanticError::VARIABLE_NOT_DEFINITELY_ASSIGNED,
                           local_decl, var -> Name());
        }
    }
}
...
void Semantic::DefiniteLocalVariableStatement(Ast* stmt)
{
    ...
    for (unsigned i = 0; i < local_decl -> NumVariableDeclarators(); i++)
    {
        ...
        if (lookup_env.option->g & JikesOption::VARS)
        {
#ifdef DUMP
...
void Semantic::DefiniteLoopBody(BitSet& starting_set)
{
    ...
    {
        Expression* name = DefiniteFinalAssignments() -> Top()[i];
...
{
    ForeachStatement* for_statement = (ForeachStatement*) stmt;
...
void Semantic::DefiniteSwitchStatement(Ast* stmt)
{
    ...
    for (i = 0; i < block_body -> NumStatements(); i++)
    {
        AstSwitchBlockStatement* switch_block_statement =
            (AstSwitchBlockStatement*) block_body -> GetStatement(i);
...
void Semantic::DefiniteMethodBody(MethodDeclaration* method_declaration)
{
    unsigned i;
    assert(FinalFields());
...
void Semantic::DefiniteConstructorBody(ConstructorDeclaration* constructor_declaration)
{
    unsigned i;
    assert(FinalFields());
#ifdef DUMP
    ...
    int size = block_body -> block_symbol -> max_variable_index +
        FinalFields() -> Length();
	int stack_size =
		constructor_declaration->method_symbol->max_block_depth;
	if (size > stack_size) {
		return;
	}
    Universe() -> Resize(size, BitSet::UNIVERSE);
	...
	auto temp = new DefiniteBlockStack(stack_size, size);
	DefiniteBlocks() = temp;
    DefinitelyAssignedVariables() -> Resize(size);
    BlankFinals() -> Resize(size, BitSet::EMPTY);
    ReachableAssignments() -> Resize(size, BitSet::EMPTY);

	auto blocks = DefiniteBlocks();
	blocks-> Push(block_body);
    ...
    {
		if (AstThisCall::ThisCallCast(block_body->explicit_constructor_opt))
			DefiniteThisCall((AstThisCall*)block_body ->
				explicit_constructor_opt);
		else if (AstSuperCall::SuperCallCast(block_body->explicit_constructor_opt))
			DefiniteSuperCall((AstSuperCall*)block_body ->
				explicit_constructor_opt);
		else
			return;
    }
