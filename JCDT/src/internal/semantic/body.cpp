// $Id: body.cpp,v 1.104 2004/12/12 04:35:52 elliott-oss Exp $
//
...
#include "JCDT_Lib/internal/semantic/SymbolTableStack.h"
...
void Semantic::ProcessBlockStatements(AstBlock* block_body)
{
    ...
    else
    {
        ...
        //
        ReachComleteAbleStatement* statement = block_body -> GetStatement(0);
        statement -> is_reachable = block_body -> is_reachable;
        ReachComleteAbleStatement* first_unreachable_statement =
            (ReachComleteAbleStatement*) (statement -> is_reachable ? NULL : statement);
        ProcessStatement(statement);
        for (unsigned i = 1; i < block_body -> NumStatements(); i++)
        {
            ReachComleteAbleStatement* previous_statement = statement;
            statement = block_body -> GetStatement(i);
            ...
            LocalBlockStack().TopBlock() -> is_reachable)
        {
            ...
            {
                ReportSemErrorWithAst(SemanticError::UNREACHABLE_STATEMENT,
                               statement);
            }
            else
            {
...
void Semantic::ProcessBlock(Ast* stmt)
{
    ...
    //     l: b = c;
    //
    if (block_body -> label_opt != nullptr)
    {
        NameSymbol* name_symbol =
           block_body -> label_opt->GetNameSymbol();
        if (LocalSymbolTable().FindLabelSymbol(name_symbol))
        {
            ReportSemErrorSimple(SemanticError::DUPLICATE_LABEL,
                                             ...
                                             bool is_static)
{
    ...
    for (TypeSymbol* type = this_type; type != nullptr; type = type -> super)
    {
        //
        ...
        for (unsigned i = 0; variable == nullptr && i < type -> NumInterfaces(); ++i)
        {
        ...
        if (variable && MemberAccessCheck(this_type, variable, base))
        {
...
{
    LocalDeclaration* local_decl = (LocalDeclaration*) stmt;
    ...
    for (unsigned i = 0; i < local_decl -> NumVariableDeclarators(); i++)
    {
        ...
        if (duplicate)
        {
            ReportSemErrorSimple(SemanticError::DUPLICATE_LOCAL_VARIABLE_DECLARATION,
        ...
        else
        {
            ...
            {
                ReportSemErrorSimple(SemanticError::UNCONVENTIONAL_VARIABLE_NAME,
                               name -> identifier_token, name_symbol -> Name());
            }
...
void Semantic::ProcessSynchronizedStatement(Ast* stmt)
{
    ...
    if (synchronized_statement -> expression -> Type() -> Primitive() ||
        synchronized_statement -> expression -> symbol == lookup_env.null_type)
    {
    ...
    if (enclosing_block_symbol -> helper_variable_index < 0)
    {
        ...
        if (ThisMethod() -> Type() != lookup_env.void_type)
        {
            if (lookup_env.IsDoubleWordType(ThisMethod() -> Type()))
                enclosing_block_symbol -> max_variable_index += 2;
...
void Semantic::CheckForAssignmentUsedAsTruthValue(Ast* expression)
{
    ...
    if (assignment_expression &&
        ...
        assignment_expression -> Type() == lookup_env.boolean_type)
    {
        ReportSemErrorWithAst(SemanticError::ASSIGNMENT_USED_AS_TRUTH_VALUE,
                       expression);
    }
}
...
{
    IfStatement* if_statement = (IfStatement*) stmt;

    ProcessExpression(if_statement -> expression);

    TypeSymbol* cond_type = if_statement -> expression -> Type();
    if (cond_type != lookup_env.boolean_type && cond_type != lookup_env.no_type)
    {
...
{
    WhileStatement* while_statement = (WhileStatement*) stmt;
    ...
    if (cond_type == lookup_env.boolean_type)
    {
    ...
    else if (cond_type != lookup_env.no_type)
    {
        ReportSemErrorWithAst(SemanticError::TYPE_NOT_BOOLEAN,
...
{
    ForStatement* for_statement = (ForStatement*) stmt;
...
{
    ForeachStatement* foreach = (ForeachStatement*) stmt;
    ...
    else
    {
        ...
        BindingSymbolTable* table = LocalSymbolTable().Top();
    ...
    else if (cond_type -> IsArray())
    {
        ...
        {
            ReportSemErrorWithAst(SemanticError::INCOMPATIBLE_TYPE_FOR_FOREACH,
                           foreach -> expression,
    ...
    else if (cond_type -> IsSubtype(lookup_env.Iterable()))
    {
        ...
        component_type = lookup_env.Object();
        if (! CanAssignmentConvertReference(index_type, component_type))
        {
    ...
    {
        ReportSemErrorWithAst(SemanticError::TYPE_NOT_ITERABLE, foreach -> expression,
                       cond_type -> ContainingPackageName(),
                       cond_type -> ExternalName());
    }
    ProcessBlock(enclosed_statement);
...
{
    SwitchStatement* switch_statement = (SwitchStatement*) stmt;
    ...
    if (! lookup_env.IsSimpleIntegerValueType(type) && type != lookup_env.no_type)
    {
    ...
    for (unsigned i = 0; i < block_body -> NumStatements(); i++)
        num_case_labels += switch_statement -> GetBlock(i) -> NumSwitchLabels();
    ...
    for (unsigned j = 0; j < block_body -> NumStatements(); j++)
    {
             ...
             k < switch_block_statement -> NumSwitchLabels(); k++)
        {
            ...
            if (switch_label -> expression_opt)
            {
                ...
                if (case_type == lookup_env.no_type)
                    continue;
				else if (!case_type){
					continue;
				}

                if (! lookup_env.IsSimpleIntegerValueType(case_type))
                {
                    ReportSemErrorWithAst(SemanticError::TYPE_NOT_INTEGER,
                                   switch_label -> expression_opt,
                                   case_type -> ContainingPackageName(),
                                   case_type -> ExternalName());
                    switch_label -> expression_opt -> symbol = lookup_env.no_type;
                }
                else if (! switch_label -> expression_opt -> IsConstant())
                {
                    ReportSemErrorWithAst(SemanticError::EXPRESSION_NOT_CONSTANT,
                                   switch_label -> expression_opt);
                    switch_label -> expression_opt -> symbol = lookup_env.no_type;
                }
                ...
                else
                {
                    IntToWstring value(DYNAMIC_CAST<IntLiteralValue*>
                                       (switch_label -> expression_opt ->
                                        value) -> value);
                    ReportSemErrorWithAst(SemanticError::VALUE_NOT_REPRESENTABLE_IN_SWITCH_TYPE,
                                   switch_label -> expression_opt,
                                   value.String(),
                                   type -> Name());
                }
            ...
            {
                ReportSemErrorWithAst(SemanticError::MULTIPLE_DEFAULT_LABEL,
                               switch_block_statement -> SwitchLabel(k));
            }
        }
            ...
            j != block_body -> NumStatements() - 1)
        {
            ...
            //
            ReportSemErrorWithAst(SemanticError::SWITCH_FALLTHROUGH,
    ...
    else
    {
        AstSwitchBlockStatement* last_switch_block_statement =
            switch_statement -> GetBlock(block_body -> NumStatements() - 1);

        assert(last_switch_block_statement -> NumStatements() > 0);

        ReachComleteAbleStatement* last_statement = last_switch_block_statement ->
            GetStatement(last_switch_block_statement -> NumStatements() - 1);
    ...
    if (switch_statement -> NumCases())
    {
        ...
        switch_statement -> GetBlock(first_case -> block_index) ->
    ...
    for (unsigned k = 1; k < switch_statement -> NumCases(); k++)
    {
        ...
        switch_statement -> GetBlock(case_elt -> block_index) ->
        ...
        if (case_elt -> value == switch_statement -> Case(k - 1) -> value)
        {
            ...
            ReportSemErrorWithAst(SemanticError::DUPLICATE_CASE_VALUE,
                           (switch_statement ->
                            GetBlock(case_elt -> block_index) ->
...
{
    DoStatement* do_statement = (DoStatement*) stmt;
    ...
    if (type != lookup_env.boolean_type && type != lookup_env.no_type)
    {
        ReportSemErrorWithAst(SemanticError::TYPE_NOT_BOOLEAN,
...
{
    BreakStatement* break_statement = (BreakStatement*) stmt;
    ...
    {
        NameSymbol* name_symbol =
            break_statement -> identifier_token_opt->GetNameSymbol();
        ...
        else
        {
            ...
            ReportSemErrorSimple(SemanticError::UNDECLARED_LABEL,
                           break_statement -> identifier_token_opt,
                            (break_statement ->identifier_token_opt->NameString()));
        }
    }
    else
    {
        ...
        else ReportSemErrorWithAst(SemanticError::MISPLACED_BREAK_STATEMENT,
                            break_statement);
    }
}
...
{
    ContinueStatement* continue_statement = (ContinueStatement*) stmt;
    ...
    {
        ReportSemErrorWithAst(SemanticError::MISPLACED_CONTINUE_STATEMENT,
                       continue_statement);
    }
    else if (continue_statement -> identifier_token_opt)
    {
        NameSymbol* name_symbol = continue_statement -> identifier_token_opt->GetNameSymbol();
        LabelSymbol* label_symbol =
            LocalSymbolTable().FindLabelSymbol(name_symbol);
        ...
        if (label_symbol)
        {
            ...
            loop_statement = label_symbol -> block -> GetStatement(0);
        }
        else
        {
            AstBlock* block_body = (AstBlock*) LocalBlockStack().TopBlock();
            ...
            ReportSemErrorSimple(SemanticError::UNDECLARED_LABEL,
                           ...
                           (continue_statement ->identifier_token_opt->NameString() ));
        }
    }
    else
    {
        AstBlock* block_body = (AstBlock*) ContinuableStatementStack().Top();
        loop_statement = block_body -> GetStatement(0);
        continue_statement -> nesting_level = block_body -> nesting_level;
    }
    ...
    if (loop_statement)
    {
        DoStatement* do_statement = DoStatement::DoStatementCast(loop_statement);
        ForStatement* for_statement = ForStatement::ForStatementCast(loop_statement);
        WhileStatement* while_statement =
            WhileStatement::WhileStatementCast(loop_statement);
        ForeachStatement* foreach_statement =
			ForeachStatement::ForeachStatementCast(loop_statement);
        ...
        else
        {
            ...
            ReportSemErrorWithAst(SemanticError::INVALID_CONTINUE_TARGET,
                           continue_statement,
                            (continue_statement ->
                                                    identifier_token_opt->NameString()));
        }
    }
}
...
{
    ReturnStatement* return_statement = (ReturnStatement*) stmt;
    MethodSymbol* this_method = ThisMethod();

    if (this_method -> name_symbol == lookup_env.nameEnv->clinit_name_symbol ||
        this_method -> name_symbol == lookup_env.nameEnv->block_init_name_symbol)
    {
        ReportSemErrorWithAst(SemanticError::RETURN_STATEMENT_IN_INITIALIZER,
                       return_statement);
    }
    else if (return_statement -> expression_opt)
    {
        Expression* expression = return_statement -> expression_opt;
        ...
        if (method_type == lookup_env.void_type ||
            this_method -> name_symbol == lookup_env.nameEnv->init_name_symbol)
        {
            ReportSemErrorWithAst(SemanticError::MISPLACED_RETURN_WITH_EXPRESSION,
                           return_statement);
        }
        else if (expression_type == lookup_env.null_type &&
                 method_type -> IsArray())
        {
            ReportSemErrorWithAst(SemanticError::EJ_RETURN_OF_NULL_ARRAY,
                           return_statement);
        }
        else if (expression_type != lookup_env.no_type)
        {
            if (method_type != expression_type)
            {
                ...
                {
                    ReportSemErrorWithAst(SemanticError::MISMATCHED_RETURN_AND_METHOD_TYPE,
                                   expression,
    ...
    else if (this_method -> Type() != lookup_env.void_type &&
             this_method -> name_symbol != lookup_env.nameEnv->init_name_symbol)
    {
        ReportSemErrorWithAst(SemanticError::MISPLACED_RETURN_WITH_NO_EXPRESSION,
                       return_statement);
    }
}
...
{
	if(lookup_env.option->jcdk_version == Option::JCDK_3_X)
	{
		return (exception != lookup_env.null_type &&
			exception != lookup_env.no_type &&
			!exception->IsSubclass(lookup_env.RuntimeException()) &&
			!exception->IsSubclass(lookup_env.Error()));
	}
	else
	{
		return (exception != lookup_env.null_type &&
...
bool Semantic::UncaughtException(TypeSymbol* exception)
{
    ...
    {
        TryStatement* try_statement = TryStatementStack()[i];
    ...
    //
    if ((this_method &&
         this_method -> Identity() == lookup_env.nameEnv->block_init_name_symbol) ||
        (ThisVariable() && ! ThisVariable() -> ACC_STATIC()))
    {
        TypeSymbol* this_type = ThisType();
        MethodSymbol* ctor =
            this_type -> FindMethodSymbol(lookup_env.nameEnv->init_name_symbol);
...
wstring Semantic::UncaughtExceptionContext()
{
    ErrorString s;
    MethodSymbol* this_method = ThisMethod();
    if (this_method)
    {
        s << " must be enclosed in a try statement that catches the "
          << "exception, ";
        if (this_method -> Identity() == lookup_env.nameEnv->clinit_name_symbol)
        {
            s << "since static initializers cannot throw checked exceptions.";
        }
        else
        {
            if (this_method -> Identity() == lookup_env.nameEnv->block_init_name_symbol)
            {
                assert(! ThisType() -> Anonymous());
                s << "or else every constructor in this class";
            }
            else if (this_method -> Identity() == lookup_env.nameEnv->init_name_symbol)
...
{
    ThrowStatement* throw_statement = (ThrowStatement*) stmt;

    ProcessExpression(throw_statement -> expression);
    TypeSymbol* type = throw_statement -> expression -> Type();

    if (type != lookup_env.no_type &&
        ! CanAssignmentConvertReference(lookup_env.Throwable(), type))
    {
        ReportSemErrorWithAst(SemanticError::EXPRESSION_NOT_THROWABLE,
                       throw_statement);
    }
    ...
    if (exception_set && type != lookup_env.null_type)
        exception_set -> AddElement(type);

    if (UncaughtException(type))
    {
		auto un_string = UncaughtExceptionContext();
		ReportSemErrorWithAst(SemanticError::UNCAUGHT_THROWN_EXCEPTION,
			throw_statement, type->ContainingPackageName(),
			type->ExternalName(), un_string.c_str());
    }
...
{
    TryStatement* try_statement = (TryStatement*) stmt;
    ...
    if (try_statement -> finally_clause_opt)
    {
        ...
        if (block_body -> NumStatements() == 0)
        {
            ReportSemErrorWithAst(SemanticError::EJ_EMPTY_FINALLY_BLOCK, block_body);
        }
    ...
    for (unsigned i = 0; i < try_statement -> NumCatchClauses(); i++)
    {
        ...
        if (! parm_type -> IsSubclass(lookup_env.Throwable()) &&
            parm_type != lookup_env.no_type)
        {
            ReportSemErrorWithAst(SemanticError::TYPE_NOT_THROWABLE, parameter,
        ...
        //
        if (lookup_env.option->pedantic && block_body -> NumStatements() == 0)
            ReportSemErrorWithAst(SemanticError::EJ_EMPTY_CATCH_BLOCK, block_body);
	...
	if (lookup_env.option->jcdk_version == Option::JCDK_3_X)
	{
		exception_set->AddElement(lookup_env.Error());
	}
    
    exception_set -> AddElement(lookup_env.RuntimeException());
    for (unsigned l = 0; l < try_statement -> NumCatchClauses(); l++)
    {
        ...
        {
            assert(exception != lookup_env.null_type);
        ...
        if (catchable_exceptions.Length() + convertible_exceptions.Length() ==
            initial_length)
        {
            ReportSemErrorWithAst(SemanticError::UNREACHABLE_CATCH_CLAUSE,
        ...
        else
        {
            ...
            if (k < l)
            {
                ...
                ReportSemErrorWithAst(SemanticError::BLOCKED_CATCH_CLAUSE,
                               clause -> formal_parameter,
                               type -> ContainingPackageName(),
                               type -> ExternalName(),
...
{
    AssertStatement* assert_statement = (AssertStatement*) stmt;

    ProcessExpression(assert_statement -> condition);

    TypeSymbol* type = assert_statement -> condition -> Type();
    if (type != lookup_env.no_type && type != lookup_env.boolean_type)
    {
        ReportSemErrorWithAst(SemanticError::TYPE_NOT_BOOLEAN,
    ...
    if (assert_statement -> message_opt)
    {
        ...
        if (assert_statement -> message_opt -> Type() == lookup_env.void_type)
...
{
    EmptyStatement* empty_statement = (EmptyStatement*) stmt;
...
TypeSymbol* Semantic::GetLocalType(TypeDeclaration* class_declaration)
{
    NameSymbol* name_symbol = 
      class_declaration -> class_body -> identifier_token->GetNameSymbol();
    TypeSymbol* type =
    ...
    wcscat(external_name, (lookup_env.option->target < JikesOption::SDK1_5
    ...
    type -> SetExternalIdentity(lookup_env.nameEnv->FindOrInsertName(external_name,
                                                         length));
...
void Semantic::ProcessClassDeclaration(Ast* stmt)
{
    ...
    TypeDeclaration* class_declaration = class_statement -> declaration;
	...
	class_declaration->symbol = inner_type;
        ...
        if (constructor)
        {
            ...
            for (unsigned i = 0; i < constructor -> NumThrows(); i++)
            {
                ...
                if (UncaughtException(exception))
                    ReportSemErrorSimple(SemanticError::UNCAUGHT_EXPLICIT_THIS_EXCEPTION,
                                   this_call -> this_token,
...
void Semantic::ProcessSuperCall(AstSuperCall* super_call)
{
    ...
    if (! super_type)
    {
        assert(this_type == lookup_env.Object());
    ...
    if (super_call -> base_opt)
    {
        ...
        if (expr_type != lookup_env.no_type)
        {
            TypeSymbol* containing_type = super_type -> EnclosingType();
            if (expr_type -> Primitive() || expr_type == lookup_env.null_type)
            {
                ReportSemErrorWithAst(SemanticError::TYPE_NOT_REFERENCE,
                               super_call -> base_opt,
                               expr_type -> ExternalName());
                super_call -> base_opt -> symbol = lookup_env.no_type;
            }
            else if (! containing_type)
            {
                if (! super_type -> Bad())
                    ReportSemErrorWithAst(SemanticError::SUPER_TYPE_NOT_INNER_CLASS,
                                   super_call -> base_opt,
            ...
            {
                ReportSemErrorWithAst(SemanticError::INVALID_ENCLOSING_INSTANCE,
                               super_call -> base_opt,
                               this_type -> ContainingPackageName(),
                               this_type -> ExternalName(),
                               containing_type -> ContainingPackageName(),
    ...
    if (constructor)
    {
        ...
        for (i = 0; i < constructor -> NumThrows(); i++)
        {
            TypeSymbol* exception = constructor -> Throws(i);
            if (UncaughtException(exception))
                ReportSemErrorWithAst(SemanticError::UNCAUGHT_EXPLICIT_SUPER_EXCEPTION,
                               super_call,
                               exception -> ContainingPackageName(),
                               exception -> ExternalName(),
                          ...
                          Tuple<AstTypeName*>* throws_list)
{
    ...
    {
        ReportSemErrorWithAst(SemanticError::NOT_A_CLASS, throw_expression,
                       throw_type -> ContainingPackageName(),
                       throw_type -> ExternalName());
    }
    else if (! throw_type -> IsSubclass(lookup_env.Throwable()))
    {
        ReportSemErrorWithAst(SemanticError::TYPE_NOT_THROWABLE, throw_expression,
                       throw_type -> ContainingPackageName(),
                       throw_type -> ExternalName());
    }
    else if (throw_type == lookup_env.Exception() ||
             throw_type == lookup_env.Throwable())
    {
        ReportSemErrorWithAst(SemanticError::EJ_OVERLY_GENERAL_THROWS_CLAUSE,
                       throw_expression);
    }
    else if (lookup_env.option->pedantic)
    {
        ...
        if (! CheckedException(throw_type))
            ReportSemErrorWithAst(SemanticError::UNCHECKED_THROWS_CLAUSE_CLASS,
                           throw_expression,
                           throw_type -> ContainingPackageName(),
                           throw_type -> ExternalName());
        else
        {
            ...
            for (unsigned i = 0; i < throws_list -> Length(); i++)
            {
                ...
                {
                    ReportSemErrorWithAst(SemanticError::DUPLICATE_THROWS_CLAUSE_CLASS,
                                   throw_expression,
                                   throw_type -> ContainingPackageName(),
                                   throw_type -> ExternalName());
                    add = false;
                ...
                {
                    ReportSemErrorWithAst(SemanticError::REDUNDANT_THROWS_CLAUSE_CLASS,
                                   throw_expression,
                                   throw_type -> ContainingPackageName(),
                                   throw_type -> ExternalName(),
...
void Semantic::ProcessMethodBody(MethodDeclaration* method_declaration)
{
    MethodSymbol* this_method = ThisMethod();
    ...
    if (method_declaration -> method_body_opt)
    {
        ...
        {
            if (this_method -> Type() == lookup_env.void_type)
            {
        ...
        {
            ReportSemErrorWithAst(SemanticError::ABSTRACT_METHOD_WITH_BODY,
                           method_declaration, this_method -> Header());
        }
    }
    else if (! (this_method -> ACC_ABSTRACT() || this_method -> ACC_NATIVE()))
    {
        ReportSemErrorWithAst(SemanticError::NON_ABSTRACT_METHOD_WITHOUT_BODY,
                       method_declaration, this_method -> Header());
    }
...
void Semantic::ProcessConstructorBody(ConstructorDeclaration* constructor_declaration)
{
    TypeSymbol* this_type = ThisType();
    MethodSymbol* this_method = ThisMethod();
    ...
    {
        super_call =
             AstSuperCall::SuperCallCast(constructor_block->explicit_constructor_opt);
        if (super_call)
            ProcessSuperCall(super_call);
        else
        {
            AstThisCall* this_call = AstThisCall::ThisCallCast(constructor_block->explicit_constructor_opt);
            if(!this_call){
				return;
            }
			assert(this_call);
    ...
    {
        auto loc = constructor_block -> LeftToken();
...
void Semantic::ProcessExecutableBodies(AstClassBody* class_body)
{
    ...
    if (! this_type -> ACC_INTERFACE())
    {
        ...
        if (class_body -> NumConstructors() == 0)
        {
            ...
            {
                ReportSemErrorWithAst(SemanticError::UNINITIALIZED_FINAL_VARIABLE,
        ...
        else
        {
            ...
            for (i = 0; i < class_body -> NumConstructors(); i++)
            {
                ...
                MethodSymbol* this_method =
                    constructor_decl -> method_symbol;
                if (! this_method)
                    continue;
                ...
                else
                {
                    *DefinitelyAssignedVariables() = initial_state;
                    ...
                    for (unsigned k = 0; k < unassigned.Length(); k++)
                    {
                        ...
                        {
                            ReportSemErrorWithAst(SemanticError::UNINITIALIZED_FINAL_VARIABLE_IN_CONSTRUCTOR,
                                           constructor_decl,
    ...
    {
        MethodDeclaration* method_decl = class_body -> Method(i);
    ...
    {
        ClassTypeDeclaration* declaration = class_body -> NestedClass(i);
    ...
    {
        InterfaceTypeDeclaration* declaration =
