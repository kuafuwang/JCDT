...
void LexStream::Dump()
{
    ...
    {
        fprintf(tokfile, "C*%5d ", com);
       // fprintf(tokfile, "%s", FileName());
        fprintf(tokfile, ", line %d.%d %d.%d: ",
                FindLine(comment_stream[com]->location),
                FindColumn(comment_stream[com]->location - 1) + 1, 
			comment_stream[com]->location, comment_stream[com]->end());
    ...
    do
    {
       ...
       // fprintf(tokfile, " %s", FileName());
        fprintf(tokfile, ", %cline %d.%d %d.%d: %s %s  ",
                (AfterEol(tok) ? '*' : ' '),
                Line(tok), (Kind(tok) == TK_EOF ? 0 : Column(tok)), start(tok),end(tok),
                token_type(Kind(tok)),
                (token_stream[tok]->Deprecated() ? "(d)" : " "));
             ...
             com > 0 && com < NumComments() && PrecedingToken(com) == tok; com++)
        {
           ...
           // fprintf(tokfile, " %s", FileName());
            fprintf(tokfile, ", line %d.%d %d.%d: ",
                    FindLine(comment_stream[com]->location),
                    FindColumn(comment_stream[com]->location - 1) + 1 , comment_stream[com]->location, comment_stream[com]->end());
            for (const wchar_t* s = CommentString(com); *s; s++)
                fprintf(tokfile, "%c", *s);
            fprintf(tokfile, "\n");
        }
