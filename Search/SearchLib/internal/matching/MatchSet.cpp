...
namespace Jikes { // Open namespace Jikes block
	...
	void MatchSet::reportMatching(AstType* field_type, JavaModel::IJavaElement* typeItem)
	{
		if (!field_type)
			return;
		if (Ast::PRIMITIVE_TYPE == field_type->class_tag)
			return;
		auto left_token = field_type->LeftToken();
		auto right_token = field_type->RightToken();
		...
		for (int i = 0; i < num; i++) 
		{
			...
			if (find_it != _matchingNodes.end())
			{
				...
				{
					// accept reference
					pattern->matchReportReference(node,typeItem, find_it->second == SearchPattern::ACCURATE_MATCH ?
						IJavaSearchResultCollector::EXACT_MATCH :
						IJavaSearchResultCollector::POTENTIAL_MATCH,this);
	...
	void MatchSet::reportMatching(TypeDeclaration* type, JavaModel::IJavaElement* parent)
	{
		...
		for (i = 0, num = class_body->NumInstanceVariables(); i < num; i++){
			...
			reportMatching(field_declaration->type, enclosingElement);
		...
		else if (TypeDeclaration::CLASS_DECL == type_kind)
		{
			...
			for (i = 0, num = actual_type->NumInterfaces(); i < num; ++i)
			{
				 superType = actual_type->Interface(i);
	...
	void MatchSet::reportTypeDeclaration(TypeDeclaration* typeDeclaration, JavaModel::IJavaElement* parent, int accuracy)
	{
		...
		if (!typeDeclaration  || !typeDeclaration->class_body)
			return;
