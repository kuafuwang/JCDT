...
public:
	...
	void AddOneDependency(const Edge& e)
	{
		...
		const wstring& first_string = e.first;
		const wstring& second_string = e.second;
...
private:
	...
	void CaculateCycles()
	{
		...
		map <int, vector< pair<int, Tstring > > > loops;

		for (std::vector<int>::size_type i = 0; i != component.size(); ++i)
			loops[component[i]].push_back(pair<int, Tstring >(discover_time[i], name[i]));

		map <int, vector< pair<int, Tstring > > >::iterator it = loops.begin();
		map <int, vector< pair<int, Tstring > > >::const_iterator itEnd = loops.end();
		_Cycles.clear();
		for (; it != itEnd; ++it) {
